...
class SmoothGravityElement : public Element {
public:
    std::string getLabel() override { return "Smooth Gravity"; }
    ...
    void action(int dir) override {
        savefile->settings.smoothGravity = !savefile->settings.smoothGravity;
        sfx(SFX_MENUCONFIRM);
    }
...
class BatteryDisplayElement : public Element {
public:
    std::string getLabel() override { return "Battery Info"; }
    ...
    void action(int dir) override {
        savefile->settings.batteryDisplay = !savefile->settings.batteryDisplay;

        sfx(SFX_MENUCONFIRM);
    }

    std::string getCurrentOption() override {
        if (savefile->settings.batteryDisplay == 0)
            return "OFF";
        else
            return "ON";
    }
...
class GraphicsScene : public Scene {
    ...
    std::list<Element*> getElementList() {
        ...
        l.push_back(new SmoothGravityElement());
        ...
        l.push_back(new BatteryDisplayElement());
#endif
    ...
    virtual std::function<Scene*()> previousScene() { return []{ return new SettingsScene(); }; };
...
class GradientEditorScene : public Scene {
    ...
    void init();
    void deinit();
    std::function<Scene*()> previousScene() { return []{ return new GraphicsScene(); }; };
