...
struct StateTypeId {
    ...
    hsm_bool operator==(const StateTypeId& rhs) const {
        HSM_ASSERT_MSG(mTypeInfo != 0,
                       "mTypeInfo was not properly initialized");
...
struct StateTypeId {
    ...
    hsm_bool operator==(const StateTypeId& rhs) const {
        HSM_ASSERT_MSG(mStateName != 0,
                       "StateTypeId was not properly initialized");
...
struct Transition {
    ...
    const OnEnterArgsFunc& GetOnEnterArgsFunc() const {
        return mOnEnterArgsFunc;
    }
...
template <typename TargetState> Transition SiblingTransition() {
...
template <typename TargetState> Transition InnerTransition() {
...
template <typename TargetState> Transition InnerEntryTransition() {
...
struct State {
    ...
    // OnEnter is invoked when a State is created; Note that GetStateMachine()
    // is valid in OnEnter. Also note that the function does not need to be
    // virtual as the state machine invokes it directly on the most-derived type
    // (not polymorphically); however, we make it virtual to avoid compiler
    // warnings about hiding base class functions.
    virtual void OnEnter() {}

    // You can also pass args to OnEnter via any of the transition functions.
    // Just make sure to declare an OnEnter that matches the argument types.
    // Since the OnEnter call is delayed, the arguments will be copied. Use
    // std::ref() to avoid the copy (but make sure the object is still valid by
    // the time OnEnter is called!).
    //
    // void OnEnter(...)
    ...
    // Called by StateMachine::ProcessStateTransitions from outermost to
    // innermost state, repeatedly until the state stack has settled (i.e. all
    // states return NoTransition). Override this function to return a state to
    // transition to, or NoTransition to remain in this state. Generally, this
    // function should avoid side-effects (updating state) as it may be called
    // several times on the same state per ProcessStateTransitions. Instead, it
    // should read state to determine whether a transition should be made.
    // Override Update instead to update the current state.
    ...
    // Usually invoked after the state stack has settled, and is where a state
    // can do it's work.
...
private:
    ...
    StateValue<T>* FindStateValueInResetterList(StateValue<T>& stateValue) {
        ...
        for (; iter != iterEnd; ++iter) {
            if (&stateValue ==
                static_cast<ConcreteStateValueResetter<T>*>(*iter)
                    ->mStateValue) {
                return &stateValue;
            }
        }
...
OnEnterArgsFunc DoGenerateOnEnterArgsFunc(Args&&... args) {
    ...
    return [args...](State* state) {
        HSM_ASSERT_MSG(state->GetStateType() == GetStateType<TargetState>(),
                       "Type of state to call OnEnter on doesn't match "
                       "original target state returned by transition");
...
public:
    ...
    template <typename InitialStateType> void Initialize(Owner* owner = 0) {
        ...
        mInitialTransition =
            SiblingTransition(GetStateFactory<InitialStateType>());
        mOwner = owner;
    }

    //@NOTE: Removing this overload as it causes ambiguity when owner is not
    // specified.
    // Can make this work using SFINAE to enable this overload when
    // StateArgsType derives from hsm::StateArgs. This would be simpler in
    // C++11.
    //
    ...
    void SetDebugTraceLevel(TraceLevel::Type trace) {
        mDebugTraceLevel = trace;
    }
    TraceLevel::Type GetDebugTraceLevel() const { return mDebugTraceLevel; }

    // Call to update the state stack (usually once per frame). This function
    // will iterate over the state stack, calling GetTransition() on each state,
    // and will perform transitions until all states return NoTransition.
    ...
    template <typename StateType> StateType* GetState() {
        return static_cast<StateType*>(
            GetState(hsm::GetStateType<StateType>()));
    }
    ...
    void Initialize(Owner* owner, const hsm_char* debugName,
                    size_t debugLevel) {
        HSM_ASSERT(mInitialTransition.IsNo());
        mInitialTransition =
            SiblingTransition(GetStateFactory<InitialStateType>());
...
inline void StateMachine::ProcessStateTransitions() {
    ...
    // After we make a transition, we must process all transitions again until
    // we get no transitions from all states on the stack.
...
inline const State* StateMachine::GetOuterState(StateTypeId stateType,
    ...
    return const_cast<StateMachine*>(this)->GetOuterState(stateType,
                                                          startDepth);
}
...
inline const State* StateMachine::GetInnerState(StateTypeId stateType,
                                                size_t startDepth) const {
    return const_cast<StateMachine*>(this)->GetInnerState(stateType,
                                                          startDepth);
}
...
inline hsm_bool StateMachine::ProcessStateTransitionsOnce() {
    // Process transitions from outermost to innermost states; if a valid
    // sibling transition is returned, we must pop inners up to and including
    // the state that returned the transition, then push the new inner. If an
    // inner transition is returned, we must pop inners up to but not including
    ...
    for (size_t depth = 0; depth < mStateStack.size(); ++depth) {
        ...
        case Transition::Inner: {
            if (State* innerState = GetStateAtDepth(depth + 1)) {
                if (transition.GetTargetStateType() ==
                    innerState->GetStateType()) {
                    ...
                    HSM_LOG_TRANSITION(1, depth + 1, HSM_TEXT("Inner"),
                                       targetState);
        ...
        case Transition::InnerEntry: {
            // If current state has no inner (is currently the innermost), then
            // push the entry state
            if (!GetStateAtDepth(depth + 1)) {
                State* targetState =
                    detail::CreateState(transition, this, depth + 1);
                HSM_LOG_TRANSITION(1, depth + 1, HSM_TEXT("Entry"),
                                   targetState);
                PushState(targetState);
                detail::InvokeStateOnEnter(transition, targetState);
                return hsm_true;
...
inline void StateMachine::Log(size_t minLevel, size_t numSpaces,
    ...
    if (static_cast<size_t>(mDebugTraceLevel) >= minLevel) {
        ...
        int offset =
            SNPRINTF(buffer, sizeof(buffer), HSM_TEXT("HSM_%lu_%s:%*s "),
