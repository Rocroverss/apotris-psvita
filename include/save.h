...
#ifdef ANDROID
    Layout onScreenLayouts[2];
    int touchSnapGrid;
#else
    int layoutPlaceHolder[10 * 2 * 5 + 1];
#endif
...
typedef struct ReplayHeader {
    u8 tag;
    u32 length; // move count
    u32 duration;
    u32 seed;
    u32 rawLength; // byte length of delta-encoded data in rawMoves
    ...
    int endLinesSent;
    int endRegretCount;
    int endTowerScrollCount;
    int endStackHeight;

    int endClears[4];
    int endTSpins;
    int endPerfectClears;
    int endMaxStreak;
    int endMaxCombo;
    int endHolds;
    int endMaxZonedLines;
    int endSecretGrade;
    int endInGameTimer;
...
typedef struct Replay {
    ReplayHeader header;
    uint8_t rawMoves[REPLAY_BUF_SIZE];
} ALIGN(4) Replay;
