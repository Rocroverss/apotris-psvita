...
public:
    ...
    std::function<Scene*()> previousScene() override { return []{ return new ModeListScene(); }; };
};
...
public:
    ...
    explicit StringSelectorElement(std::list<std::string> list) {
        optionList = std::move(list);
...
public:
    ...
    explicit IntSelectorElement(std::list<int> list) {
        optionList = std::move(list);
        options = optionList.size();
    }
...
public:
    ...
    std::list<Element*> getElementList() {
        return {
            new StringSelectorElement("Type",
                                      {"Normal", "Zone", "Tower", "Cascade"}),
    ...
    EntryBoard* getBoard(int submode, int goal) {
        if (submode == 0) {
            ...
            return &savefile->boards.cascade[goal];
        } else {
...
public:
    ...
    std::list<Element*> getElementList() {
        return {
            new TupleSelectorElement("Minutes", {{3 * FRAMES_PER_MIN, "3"},
                                                 {5 * FRAMES_PER_MIN, "5"},
                                                 {10 * FRAMES_PER_MIN, "10"}}),
        };
    };
...
public:
    ...
    std::string getCurrentOption() override {
        ...
        } else if (selector->getValue() == 1) {
            return std::to_string(savefile->boards.zenZone);
        } else if (selector->getValue() == 2) {
            return std::to_string(savefile->boards.zenTower);
        } else {
            return std::to_string(savefile->boards.zenCascade);
        }
    }
...
public:
    ...
    std::list<Element*> getElementList() {
        StringSelectorElement* sel = new StringSelectorElement(
            "Type", {"Normal", "Zone", "Tower", "Cascade"});
        return {
            sel,
...
public:
    BlockEngine::Modes getMode() override { return BlockEngine::BATTLE; };

    int winTargetIndex = 1; // 0=3, 1=5, 2=7
    const int winTargets[3] = {3, 5, 7};
    bool choosingWinTarget = false;

    std::list<Element*> getElementList() override { return {}; };
    ...
    std::string name() override { return "Multi Battle"; };

    void showReadyPlayers();
    void showWinTargetSelection();
    void showWinTargetCursor();
    void handleConnectionDisplay();
    void handleReadyStatus();

    void hideReadyPlayers();

#ifdef MULTIBOOT
    std::function<Scene*()> previousScene() override { return []{ return new MultBattleScene(); }; };
#endif
...
#else
    bool matchMake = false;

    explicit MultBattleScene();
    explicit MultBattleScene(std::string roomId);
#endif
};
...
#ifdef GBA
class SinglePakWirelessScene : public PlayOptionScene {
private:
    enum MultibootState { NOT_READY, READY, FAILED_INIT, NONE };
    MultibootState multibootState = NOT_READY;
    u8 lastPlayerCount = 255;
    u8 lastProgress = 255;
    Link::AsyncMultiboot::Result lastResult =
        Link::AsyncMultiboot::Result::NONE;
...
public:
    ...
    std::string name() override { return "Wireless Pak"; };

    void pumpMultibootFeed();
    void startMultibootTransfer();
};

class SinglePakWiredScene : public PlayOptionScene {
private:
    enum MultibootState { NOT_READY, READY, FAILED_INIT, NONE };
    MultibootState multibootState = NOT_READY;
    u8 lastPlayerCount = 255;
...
public:
    ...
    std::string name() override { return "Wired Pak"; };
