...
extern uint16_t* getShape(int piece, int rotation, int rotationSystem);
...
public:
    ...
    Score(const Score& oldScore) {
        ...
        cascadeComplexity = oldScore.cascadeComplexity;
...
public:
    ...
    SoundFlags(const SoundFlags& oldFlags) {
        ...
        cascade = oldFlags.cascade;
        attackApplied = oldFlags.attackApplied;
        attackReceived = oldFlags.attackReceived;
    }
};
...
public:
    ...
    Connection(int x, int y, int blocks) {
        this->x = x & 0xff;
        this->y = y & 0xff;
        this->blocks = blocks & 0xffff;
    }

    int packed() {
        int result = 0;
        memcpy32(&result, this, 1);
...
private:
    ...
    int checkIntegrity();
    bool cascade();
    void calculateColumnHeights(int start);
...
public:
    ...
    std::vector<int> getIncomingAttacks() const;
    Move findBestDrop();
    ...
    int gameMode;
    SoundFlags sounds;
    Score previousClear = Score(0, 0, 0, 0, 0, 0, 0, 1, Drop());
    int timer = 0;
    int refresh = 0;
    int won = 0;
    int goal = 40;
    ...
    std::list<Timestamp> replay; // max moves is REPLAY_MAX_MOVES
...
public:
    ...
    void executeMove(const Move& move) const;
