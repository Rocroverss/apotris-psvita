#include "def.h"
#include "platform.hpp"
#include "rumble.h"
#include "rumblePatterns.hpp"
#include "scene.hpp"
#include "sceneModes.hpp"
#include "tetromino.hpp"
// #include "soundbank.h"
#include "blockEngine.hpp"
#include "logging.h"
#include "text.h"
#include <cstring>
#include <map>
#include <posprintf.h>
#include <string>

#ifdef PC
#include <fstream>
#endif

#include "multiplayerClasses.h"
#include "sceneMultiplayerResults.hpp"
#include "qrcodegen.h"

using namespace BlockEngine;

void endAnimation();
void showScore();
int onRecord();
std::string nameInput(int, bool* = nullptr);
void qrScene();

void saveReplay();

int mode = 0;
bool showingStats = false;

bool saveExists = false;
BlockEngine::Game* quickSave = nullptr;
#ifndef MULTIBOOT
BlockEngine::Bot* testBot = nullptr;
#endif

int igt = 0;

bool ended = false;

#ifndef MULTIBOOT
const std::string modeStrings[] = {
    "Marathon", "Sprint",  "Dig",    "Battle",   "Ultra", "Blitz", "Combo",
    "Survival", "Classic", "Master", "Training", "Zen",   "Death",
};

const std::string modeOptionStrings[][5] = {
    {"150", "200", "300", "Endless"},
    {"20", "40", "100"},
    {"10", "20", "100"},
    {"EASY", "MEDIUM", "HARD", "V.HARD", "INSANE"},
    {"3", "5", "10"},
    {""},
    {""},
    {"EASY", "MEDIUM", "HARD"},
    {"", ""},
    {"Normal", "Classic"},
    {""},
    {""},
    {"Normal", "Classic"},
};
#endif

void GameScene::endScreen() {
    const int optionsHeight = 11;
    int max = 3;
    int selection = 0;

    // - calculate pps
    FIXED t = gameSeconds * float2fx(0.0167f);
    FIXED pps = fxdiv(int2fx(game->pieceCounter), (t));

    std::string ppsStr = std::to_string(fx2int(pps)) + ".";

    igt = game->inGameTimer;

    int fractional = pps & 0xff;
    for (int i = 0; i < 2; i++) {
        fractional *= 10;
        ppsStr += std::to_string('0' + (fractional >> 8));
        fractional &= 0xff;
    }

    std::string totalTime = timeToString(gameSeconds, false);

    stopMusic();

    int prevBld = blendInfo;

    int record = endScreenSetup();
    const bool finalMatch = multiplayer && multiplayerLink->matchComplete;

    if (multiplayer)
#ifndef MULTIBOOT
        max = finalMatch ? 1 : 2;
#else
        max = 1;
#endif

    playSongRandom(0);

    if (savefile->settings.announcer) {
        if (game->gameMode == ULTRA || game->gameMode == BLITZ) {
            if (game->won == 1) {
                sfx(SFX_TIME);
                rumblePattern(RUMBLE_TSPIN);
            } else if (game->lost == 1) {
                sfx(SFX_GAMEOVER);
                rumblePattern(RUMBLE_YOULOSE);
            }
        } else if (game->gameMode != BATTLE) {
            if (game->gameMode == MASTER && game->won == 1 &&
                game->level / 100 == 5) {
                sfx(SFX_TIME);
                rumblePattern(RUMBLE_TSPIN);
            } else if (game->won == 1) {
                sfx(SFX_CLEAR);
                rumblePattern(RUMBLE_YOUWIN);
            } else if (game->lost == 1) {
                sfx(SFX_GAMEOVER);
                rumblePattern(RUMBLE_YOULOSE);
            }
        } else {
            if (game->won == 1) {
                sfx(SFX_YOUWIN);
                rumblePattern(RUMBLE_YOUWIN);
            } else if (game->lost == 1) {
                sfx(SFX_YOULOSE);
                rumblePattern(RUMBLE_YOULOSE);
            }
        }
    }

    showStats(showingStats, totalTime, ppsStr, true);

    if (!replaying) {
#ifndef NO_DIAGNOSE
        // Fill replay to the limit to test memory pressure during
        // "view replay" scene transition on GBA.
        game->fillReplayToLimit();
#endif
        saveReplay();
    } else {
        delete currentReplayHeader;
        currentReplayHeader = nullptr;

        replayBytes = nullptr;
        replayCount = 0;

        replaying = false;
    }

    int cursorFloat = 0;
    OBJ_ATTR* cursorSprite;

    loadSpriteTiles(16 * 7, blockSprite, 1, 1);
    cursorSprite = &obj_buffer[1];
    sprite_hide(cursorSprite);

    std::string placeString;
    if (record != -1 && game->gameMode != BATTLE &&
        (game->won || game->gameMode == MARATHON || game->gameMode >= ULTRA)) {
        switch (record) {
        case 0:
            placeString = "1st";
            break;
        case 1:
            placeString = "2nd";
            break;
        case 2:
            placeString = "3rd";
            break;
        case 3:
            placeString = "4th";
            break;
        case 4:
            placeString = "5th";
            break;
        }
    } else if (game->gameMode == BATTLE) {
        switch (rank - 1) {
        case 0:
            placeString = "1st";
            break;
        case 1:
            placeString = "2nd";
            break;
        case 2:
            placeString = "3rd";
            break;
        case 3:
            placeString = "4th";
            break;
        case 4:
            placeString = "5th";
            break;
        }
    }

    if (placeString != "") {
        placeString += " Place";
        naprint(placeString, 15 * 8 - getVariableWidth(placeString) / 2, 5 * 8);
    }

    MenuKeys k = savefile->settings.menuKeys;

    bool shownOptions = false;
    while (closed()) {
        if (handleMultiplayer(false)) {
            return;
        }

        vsync();
        key_poll();

        if (!multiplayerLink->stateMachine
                 .IsInState<MultiplayerStates::Playing>()) {
            if (!shownOptions) {
                showEndScreenOptions();
                shownOptions = true;
            }
            showScore();
        }

        cursorFloat += 6;
        if (cursorFloat >= 512)
            cursorFloat = 0;
        int offset = (sinLut(cursorFloat) * 2) >> 12;
        FIXED scale = float2fx((1.0 - ((float)0.1 * offset)));

        sprite_unhide(cursorSprite, 0);
        sprite_set_attr(cursorSprite, ShapeSquare, 0, 7 * 16, 5, 0);
        sprite_enable_affine(cursorSprite, 6, true);
        sprite_set_size(cursorSprite, scale, 6);

        int x = 240 / 2 - ((20) + offset + 4) - 16;

        const int optionOffset = finalMatch ? 3 : 0;
        sprite_set_pos(cursorSprite, x,
                       ((optionsHeight + optionOffset + selection * 3) * 8) -
                           5);

        int counter = 0;

        wordSprites[1].show(12 * 8,
                            (optionsHeight + optionOffset + counter) * 8,
                            14 + (selection == 0));
        wordSprites[2].show(14 * 8,
                            (optionsHeight + optionOffset + counter + 1) * 8,
                            14 + (selection == 0));
        counter += 3;

        if (!multiplayer) {
            wordSprites[3].show(12 * 8, (optionsHeight + counter) * 8,
                                14 + (selection == 1));
            wordSprites[4].show(14 * 8, (optionsHeight + counter + 1) * 8,
                                14 + (selection == 1));
            counter += 3;
        }

        if (!finalMatch) {
            wordSprites[5].show(12 * 8, (optionsHeight + counter) * 8,
                                14 + (selection == 1 + !multiplayer));
            wordSprites[6].show(14 * 8, (optionsHeight + counter + 1) * 8,
                                14 + (selection == 1 + !multiplayer));
        }

        if (multiplayerStart) {
            multiplayerStart = false;
            startGame(nextSeed);
            changeScene([]() { return new GameScene(); });
            break;
        }

        if (key_hit(k.confirm) || key_hit(k.pause)) {
            if (selection == 0) {
                if (finalMatch) {
                    sfx(SFX_MENUCONFIRM);
                    changeScene([]() { return new MultiplayerResultsScene(); },
                                Transitions::FADE);
                    return;
                }

                shake = 0;

                if (!multiplayer) {
                    sfx(SFX_MENUCONFIRM);
                    startGame();
                    changeScene([]() { return new GameScene(); });
                    break;
                } else {
                    if (multiplayerLink->stateMachine.IsInState<
                            MultiplayerStates::PlayersAwaitReady>()) {
                        // Mark that we are ready
                        sfx(SFX_MENUCONFIRM);
                        multiplayerLink->readyUp();
                    } else {
                        sfx(SFX_MENUCANCEL);
                    }
                }
            } else if (selection == max - 1) {
                sfx(SFX_MENUCANCEL);
                enableBlend(prevBld);
                buildBG(3, 0, 27, 0, 1, true);
                if (multiplayer) {
                    multiplayerLink->deactivate(true);
                    multiplayer = false;
                }
#ifndef MULTIBOOT
                changeScene([]() { return new MainMenuScene; },
                            Transitions::FADE);
#else
                changeScene([]() { return new MultiplayerProtocolScene; },
                            Transitions::FADE);
#endif
                return;
            } else if (selection == 1) {
                sfx(SFX_MENUCANCEL);
                enableBlend(prevBld);
                path.clear();
                path.emplace_back("Play");
                clearText();
                buildBG(3, 0, 27, 0, 1, true);
                sceneSwitcher(modeToString(previousGameOptions->mode));
                nextSeed = 0;
                return;
            }
        }

        if (key_hit(k.up)) {
            if (selection == 0)
                selection = max - 1;
            else
                selection--;
            sfx(SFX_MENUMOVE);
        }

        if (key_hit(k.down)) {
            if (selection == max - 1)
                selection = 0;
            else
                selection++;
            sfx(SFX_MENUMOVE);
        }

        if (key_hit(k.special1)) {
            sfx(SFX_MENUCONFIRM);
            showingStats = !showingStats;
            clearSmallText();
            showStats(showingStats, totalTime, ppsStr, true);
        }

        if (!multiplayer && key_hit(k.special2)) {
            if (game->replayElligible) {
                sfx(SFX_MENUCONFIRM);
                loadReplay();
                break;
            } else {
                sfx(SFX_MENUCANCEL);
            }
        }

        if (multiplayer) {
            showReadyPlayers();
        }

        updateGrid();
        updateFluid();
        drawGrid();

        showSprites(128);
    }

    buildBG(3, 0, 27, 0, 1, true);
    enableBlend(prevBld);
}

void endAnimation() {
    ended = true;
    shake = 0;
    push = 0;

    clearText();
    clearSprites(128);
    showHold();

    showQueue(1);
    drawFrame(-1);

    showSprites(128);

    setLayerScroll(1, 0, 0);
    setLayerScroll(2, 0, 0);

    enableLayerWindow(1, 0, 0, 240, 160, false);
    enableLayerWindow(2, 0, 0, 240, 160, false);

#if defined(TE) || defined(N3DS)
    setSpriteMaskRegion(shake);
#endif

    rumblePatternStop();

    sfx(SFX_END);

    for (int i = 0; i < 40; i++) {
        multiplayerLink->sync();
        closed();
        vsync();
        drawGrid();
        showClearText();

        updateFluid();
        checkPeek();

        if (i % 2 != 0)
            continue;

        showBackground(i / 2);
    }

    int counter = 0;
    while (!floatingList.empty() && closed()) {
        multiplayerLink->sync();
        vsync();
        showClearText();

        if (++counter > 120)
            break;
        updateFluid();
        drawGrid();
        checkPeek();
    }
    clearText();

    setLayerScroll(1, 0, 0);
    setLayerScroll(2, 0, 0);
}

void showScore() {
    if (game->gameMode == BATTLE) {
        if (game->lost) {
            aprint("YOU LOSE", 11, 3);
        } else {
            aprint("YOU WIN!", 11, 3);
        }

        aprint("Lines Sent", 10, 7);
        aprintf(game->linesSent, 14, 9);

    } else if (game->gameMode == MASTER) {
        int grade = game->grade + game->coolCount + (int)game->creditGrade;
        if (grade > 32)
            grade = 32;

        std::string gradeText = GameInfo::masterGrades[grade];

        if (gradeText[0] == ' ')
            gradeText.erase(0, 1);

        gradeText = "Grade: " + gradeText;

        if (game->won && game->level / 100 == 5) {
            aprint("TIME!", 13, 3);
        } else if (game->won && game->level == 999) {
            aprint("CLEAR!", 12, 3);
        } else {
            aprint("GAME OVER", 11, 3);
        }

        aprint(gradeText, 15 - ((int)gradeText.size() / 2), 7);
    } else if (game->gameMode == DEATH) {

        int section = game->level / 100;

        if (game->won && (section == 5 || section == 10)) {
            aprint("TIME!", 13, 3);
        } else if (game->won && game->level == 1300) {
            aprint("CLEAR!", 12, 3);
        } else {
            aprint("GAME OVER", 11, 3);
        }

        int grade = max(section - game->regretCount, 0);

        if (grade > 0 && grade <= 13) {
            std::string gradeText = "Grade: S" + std::to_string(grade);

            aprint(gradeText, 15 - ((int)gradeText.size() / 2), 7);
        }
    } else if (game->gameMode == MARATHON || game->lost ||
               game->gameMode >= ULTRA ||
               (game->gameMode == DIG && game->subMode == 1)) {
        std::string score;

        if (game->gameMode == DIG) {
            if (game->subMode == 1)
                score = std::to_string(game->pieceCounter);
        } else if (game->gameMode == ZEN && game->subMode == 2) {
            score = std::to_string(game->towerScrollCount +
                                   (game->lengthY - game->stackHeight));
        } else if (game->gameMode != COMBO)
            score = std::to_string(game->score);
        else
            score = std::to_string(game->statTracker.maxCombo);

        if (game->lost)
            aprint("GAME OVER", 11, 3);
        else {
            if (game->gameMode != ULTRA && game->gameMode != BLITZ)
                aprint("CLEAR!", 12, 3);
            else
                aprint("TIME!", 13, 3);
        }

        if (game->gameMode == MARATHON || game->gameMode == ULTRA ||
            game->gameMode == BLITZ || game->gameMode == COMBO ||
            game->gameMode == DIG || game->gameMode == CLASSIC)
            aprint(score, 15 - ((int)score.size() / 2), 7);
        else if (game->gameMode == SURVIVAL)
            aprint(timeToString(gameSeconds, false), 11, 7);
    } else {
        aprint("CLEAR!", 12, 3);

        aprint(timeToString(gameSeconds, false), 11, 7);
    }
}

void GameScene::showStats(bool moreStats, const std::string& time,
                          const std::string& pps, bool end) const {
    if (!moreStats) {
        aprints("Press " +
                    getStringFromKey(savefile->settings.menuKeys.special1),
                0, 0, 2);
        aprints("for Stats", 0, 7, 2);

        if (end && !(multiplayer || game->gameMode == BATTLE)) {
            if (game->replayElligible) {
                aprints("Press " + getStringFromKey(
                                       savefile->settings.menuKeys.special2),
                        0, 21, 2);
                aprints("for Replay", 0, 28, 2);
            } else {
                aprints("Replay", 0, 21, 1);
                aprints("not available", 0, 28, 1);
            }
        } else if (game->gameMode != TRAINING) {
            std::string keyString =
                getStringFromKey(savefile->settings.menuKeys.special3);

            std::transform(keyString.begin(), keyString.end(),
                           keyString.begin(), ::toupper);

            aprints("Press " + keyString, 0, 21, 2);
            aprints("to Skip Track", 0, 28, 2);
        }

        if (!end && savefile->settings.journey) {
            aprints("Hold " +
                        getStringFromKey(savefile->settings.menuKeys.special1),
                    0, 42, 2);
            aprints("to save graphics", 0, 49, 2);

            aprints("Hold " +
                        getStringFromKey(savefile->settings.menuKeys.special2),
                    0, 61, 2);
            aprints("to reset graphics", 0, 70, 2);
        }

        return;
    }

    int counter = 0;

    int g = game->gameMode;

    aprints("Time: " + time, 0, 7 * counter++, 2);
    if (!proMode)
        aprints("IGT: " + timeToString(igt, false), 0, 7 * counter++, 2);

    if (g == MARATHON || g == BLITZ || g == CLASSIC) {
        if (g != BLITZ)
            aprints("Start Level: " + std::to_string(game->initialLevel), 0,
                    7 * counter++, 2);

        aprints("Final Level: " + std::to_string(game->level), 0, 7 * counter++,
                2);
        aprints("Score: " + std::to_string(game->score), 0, 7 * counter++, 2);
    } else if (g == ULTRA) {
        aprints("Score: " + std::to_string(game->score), 0, 7 * counter++, 2);
    } else if (g == MASTER || g == DEATH) {
        aprints("Level: " + std::to_string(game->level), 0, 7 * counter++, 2);
    }

    if ((g == MARATHON && game->subMode == 2) ||
        (g == ZEN && game->subMode == 2)) {
        int height =
            game->towerScrollCount + (game->lengthY - game->stackHeight);
        aprints("Height: " + std::to_string(height), 0, 7 * counter++, 2);
    }

    aprints("Lines: " + std::to_string(game->linesCleared), 0, 7 * counter++,
            2);
    aprints("Pieces: " + std::to_string(game->pieceCounter), 0, 7 * counter++,
            2);

    aprints("PPS: " + pps, 0, 7 * counter++, 2);

    if (game->rotationSystem == SRS)
        aprints("Finesse: " + std::to_string(game->finesseFaults), 0,
                7 * counter++, 2);

    aprints("Singles: " + std::to_string(game->statTracker.clears[0]), 0,
            7 * counter++, 2);
    aprints("Doubles: " + std::to_string(game->statTracker.clears[1]), 0,
            7 * counter++, 2);
    aprints("Triples: " + std::to_string(game->statTracker.clears[2]), 0,
            7 * counter++, 2);
    aprints("Quads: " + std::to_string(game->statTracker.clears[3]), 0,
            7 * counter++, 2);

    if (game->gameMode != CLASSIC) {
        aprints("T-Spins: " + std::to_string(game->statTracker.tspins), 0,
                7 * counter++, 2);

        if (savefile->settings.aspectRatio == 0)
            aprints("Perfect Clears: " +
                        std::to_string(game->statTracker.perfectClears),
                    0, 7 * counter++, 2);
        else if (savefile->settings.aspectRatio == 1)
            aprints("PCs: " + std::to_string(game->statTracker.perfectClears),
                    0, 7 * counter++, 2);

        aprints("Max Streak: " + std::to_string(game->statTracker.maxStreak), 0,
                7 * counter++, 2);
        aprints("Max Combo: " + std::to_string(game->statTracker.maxCombo), 0,
                7 * counter++, 2);
        aprints("Times Held: " + std::to_string(game->statTracker.holds), 0,
                7 * counter++, 2);
    }

    if (game->gameMode == MASTER) {
        aprints("Section Cools: " + std::to_string(game->coolCount), 0,
                7 * counter++, 2);
        aprints("Section Regrets: " + std::to_string(game->regretCount), 0,
                7 * counter++, 2);
    }

    if (game->statTracker.maxZonedLines > 0) {
        aprints("Max MultiClear: " +
                    std::to_string(game->statTracker.maxZonedLines),
                0, 7 * counter++, 2);
    }

    if (game->statTracker.maxCascade > 0) {
        aprints("Max Cascade: " + std::to_string(game->statTracker.maxCascade),
                0, 7 * counter++, 2);
    }

    aprints("Times Paused: " + std::to_string(timesPaused), 0, 7 * counter++,
            2);

    if (game->statTracker.secretGrade > 0) {
        std::string str =
            "Secret Grade: " +
            (std::string)
                GameInfo::secretGrades[game->statTracker.secretGrade - 1];

        aprints(str, 0, 7 * counter++, 2);
    }
}

int GameScene::pauseMenu() {
    int selection = 0;
    int maxSelection;

    int optionsHeight = 10;
    int optionsCounter = 0;

    timesPaused++;

    clearText();
    setSmallTextArea(220, 1 + (savefile->settings.aspectRatio != 0), 1, 12, 20);

    for (auto& wordSprite : wordSprites)
        wordSprite.hide();

    int prevBld = blendInfo;
    enableBlend((1 << 6) + (0b11101 << 9) + (1 << 3));
    setTiles(27, 0, 32 * 32, tileBuild(34, false, false, 0));
    buildBG(3, 0, 27, 0, 0, 0);
    clearTilemap(25);

    drawFrame(1);
    if (game->zoneTimer)
        frameSnow(1);

    setLayerScroll(2, 0, 0);

    // hide Sprites
    hideMinos();
    sprite_hide(&obj_buffer[23]);  // hide meter
    sprite_hide(&obj_buffer[24]);  // hide finesse combo counter
    sprite_hide(&obj_buffer[25]);  // hide enemyBoard
    sprite_hide(&obj_buffer[127]); // hide battery sprite
    for (int i = 0; i < 3; i++)
        sprite_hide(&obj_buffer[16 + i]);

    showSprites(128);

    rumblePatternStop();

    // calculate pps
    FIXED t = gameSeconds * float2fx(0.0167f);
    FIXED pps = 0;
    if (t > 0)
        pps = fxdiv(int2fx(game->pieceCounter), (t));

    std::string ppsStr = std::to_string(fx2int(pps)) + ".";

    int fractional = pps & 0xff;
    for (int i = 0; i < 2; i++) {
        fractional *= 10;
        ppsStr += std::to_string('0' + (fractional >> 8));
        fractional &= 0xff;
    }

    std::string totalTime = timeToString(gameSeconds, false);
    igt = game->inGameTimer;

    showModeText();

    int pendingSong = -1;

    auto showNowPlaying = [](int songIndex) {
        std::string trackName = getDisplayTrackName(songIndex);
        if (trackName.empty())
            return;

        int rightEdge = (savefile->settings.aspectRatio) ? (240 - 13) : 240;

        std::string label = "Now playing:";
        int lx = rightEdge - getVariableWidth(label);
        if (lx < 0)
            lx = 0;
        naprint(label, lx, 17 * 8);

        int tx = rightEdge - getVariableWidth(trackName);
        if (tx < 0)
            tx = 0;
        naprint(trackName, tx, 18 * 8);
    };

    // Show currently playing track name in bottom right
    if (game->gameMode != TRAINING && savefile->settings.volume > 0)
        showNowPlaying(currentlyPlayingSong);

    bool shown = false;
    int saveHoldTimer = 0;
    int resetHoldTimer = 0;
    const int holdThreshold = 30;

    int confirmHoldTimer = 0;
    int confirmHoldAction = -1; // -1 = inactive, 1 = restart, 3 = quit
    OBJ_ATTR* confirmMeterSprite = &obj_buffer[23];

    bool count = true;
    const bool shortGame = gameSeconds < 5 * 60 * 60;

    auto restartGame = [&]() {
        addGameStats();
        startGame();
        changeScene([]() { return new GameScene(); });
        paused = false;
        count = false;
        sfx(SFX_MENUCONFIRM);
    };

    auto quitGame = [&]() {
        enableBlend(prevBld);
        sfx(SFX_MENUCANCEL);
        buildBG(3, 0, 27, 0, 1, true);
        addGameStats();
        playSongRandom(0);

#ifdef MULTIBOOT
        changeScene([]() { return new MultBattleScene(); },
                    Transitions::FADE);
#else
        changeScene([]() { return new MainMenuScene(); },
                    Transitions::FADE);
#endif
    };

    int cursorFloat = 0;
    OBJ_ATTR* cursorSprites[2];

    loadSpriteTiles(16 * 7, blockSprite, 1, 1);
    for (int i = 0; i < 2; i++) {
        cursorSprites[i] = &obj_buffer[1 + i];
        sprite_set_attr(cursorSprites[i], ShapeSquare, 0, 7 * 16, 5, 0);
        sprite_enable_affine(cursorSprites[i], i, true);
        sprite_hide(cursorSprites[i]);
    }

    MenuKeys k = savefile->settings.menuKeys;

    while (closed()) {
        if (!onStates) {
            maxSelection = 4;
            if (game->gameMode != TRAINING)
                maxSelection--;

            if (maxSelection == 3) {
                optionsHeight = 11;
            }
        } else
            maxSelection = 3;

        vsync();
        key_poll();
#ifdef GBA
        if (sleep_combo_check(key_curr_state())) {
            // Re-poll keys to avoid triggering stats/track skip.
            // I'm not a fan of this but it works with the current scene
            // infrastructure.
            key_poll();
        }
#endif

        wordSprites[0].setText("PAUSE!");
        wordSprites[0].show(15 * 8 - wordSprites[0].width / 2, 4 * 8, 15);

        optionsCounter = 0;

        auto showOption = [this, &optionsCounter, optionsHeight,
                           selection](std::string str) {
            wordSprites[1 + optionsCounter].setText(str);
            wordSprites[1 + optionsCounter].show(
                15 * 8 - wordSprites[1 + optionsCounter].width / 2,
                (optionsHeight + optionsCounter * 2) * 8,
                15 - !(selection == optionsCounter));
            optionsCounter++;
        };

#ifdef GBA
        if (!multiplayer) {
            std::string keyString =
                (sleep_combo == 0) ? "Unbound" : getStringFromKey(sleep_combo);

            std::replace(keyString.begin(), keyString.end(), ' ', '+');

            aprints("Sleep/Wake: " + keyString, 0, 140, 2);
        }
#endif

        if (!onStates) {

            showOption("Resume");
            showOption("Restart");

            if (game->gameMode == TRAINING)
                showOption("Saves");

            showOption("Quit");

            // Hold-to-confirm processing for Restart/Quit
            if (confirmHoldAction >= 0) {
                if (key_is_down(k.confirm)) {
                    confirmHoldTimer++;

                    // Play progress sfx every 1/8 second (~8 frames)
                    if (confirmHoldTimer % 8 == 0) {
                        sfx(SFX_SHIFT2);
                    }

                    // Show zone meter progress sprite
                    int progress = confirmHoldTimer * 12 / 60;
                    if (progress > 12)
                        progress = 12;

                    int meterX = (savefile->settings.aspectRatio != 0) ? 13 : 4;
                    sprite_set_attr(confirmMeterSprite, ShapeSquare, 2, 256 + 3,
                                    12, 0);
                    sprite_enable_mosaic(confirmMeterSprite);
                    sprite_set_pos(confirmMeterSprite, meterX, 160 - 36);
                    sprite_unhide(confirmMeterSprite, 0);

                    for (int i = 0; i < 12; i++) {
                        if (i < progress)
                            setPaletteColor(12 + 16, 4 + i, 0x7fff, 1);
                        else
                            setPaletteColor(12 + 16, 4 + i, 0x0c63, 1);
                    }

                    // Action completes after 1 second (60 frames)
                    if (confirmHoldTimer >= 60) {
                        sprite_hide(confirmMeterSprite);
                        if (confirmHoldAction == 1) {
                            restartGame();
                            return 0;
                        } else if (confirmHoldAction == 3) {
                            quitGame();
                            return 1;
                        }
                    }
                } else {
                    // Button released, cancel hold
                    confirmHoldTimer = 0;
                    confirmHoldAction = -1;
                    sprite_hide(confirmMeterSprite);
                }
            } else if (key_hit(k.confirm)) {
                int n = selection;
                if (!(game->gameMode == TRAINING) && selection >= 2)
                    n++;

                if (n == 0) {
                    sfx(SFX_MENUCONFIRM);
                    clearText();
                    paused = false;
                    break;
                } else if (n == 1) {
                    if (shortGame) {
                        restartGame();
                        return 0;
                    }

                    // Start hold-to-confirm for Restart
                    confirmHoldAction = 1;
                    confirmHoldTimer = 0;
                } else if (n == 2) {
                    onStates = true;
                    selection = 0;
                    clearText();
                } else if (n == 3) {
                    if (shortGame) {
                        quitGame();
                        return 1;
                    }

                    // Start hold-to-confirm for Quit
                    confirmHoldAction = 3;
                    confirmHoldTimer = 0;
                }
            }
        } else {
            optionsCounter = 0;

            showOption("Load");
            showOption("Save");
            showOption("Back");

            for (int i = optionsCounter + 1; i < 6; i++)
                wordSprites[i].hide();

            if (key_hit(k.confirm)) {
                if (selection == 0) {
                    if (saveExists) {
                        delete game;
                        game = new Game(quickSave);
                        game->setTuning(getTuning());
                        floatingList.clear();
                        placeEffectList.clear();
                        clearGlow();

                        if (savefile->settings.colors == 3)
                            setPalette();

#ifndef MULTIBOOT
                        if (enableBot) {
                            delete testBot;
                            testBot = new Bot(game);
                        }
#endif

                        aprint("Loaded!",
                               22 - (savefile->settings.aspectRatio == 1), 18);
                        sfx(SFX_MENUCONFIRM);
                    } else {
                        sfx(SFX_MENUCANCEL);
                    }
                } else if (selection == 1) {
                    if (saveExists)
                        delete quickSave;
                    quickSave = new Game(game);
                    saveExists = true;

                    aprint(" Saved!",
                           22 - (savefile->settings.aspectRatio == 1), 18);
                    sfx(SFX_MENUCONFIRM);
                } else if (selection == 2) {
                    sfx(SFX_MENUCONFIRM);
                    showStats(showingStats, totalTime, ppsStr, false);
                    onStates = false;
                    selection = 0;
                }
            }
        }

        int len = wordSprites[selection + 1].width;

        cursorFloat += 6;
        if (cursorFloat >= 512)
            cursorFloat = 0;
        int offset = (sinLut(cursorFloat) * 2) >> 12;
        FIXED scale = float2fx((1.0 - ((float)0.1 * offset)));

        for (int i = 0; i < 2; i++) {
            sprite_unhide(cursorSprites[i], 0);
            sprite_set_attr(cursorSprites[i], ShapeSquare, 0, 7 * 16, 5, 0);
            sprite_enable_affine(cursorSprites[i], i, true);
            sprite_set_size(cursorSprites[i], scale, i);

            int x = 240 / 2 - ((len + 8) / 2 + offset + 4) * ((i) ? -1 : 1) - 8;

            sprite_set_pos(cursorSprites[i], x,
                           ((optionsHeight + selection * 2) * 8) - 5);
        }

        if (key_hit(k.pause) && confirmHoldAction < 0) {
            sfx(SFX_MENUCONFIRM);
            clearText();
            // updateText();
            paused = false;
            break;
        }

        if (key_hit(k.special3) && confirmHoldAction < 0) {
#ifndef NO_DIAGNOSE
            qrScene();
#endif
            if (game->gameMode != TRAINING) {
                sfx(SFX_MENUCONFIRM);

                int from =
                    (pendingSong >= 0) ? pendingSong : currentlyPlayingSong;
                int next = getNextSong(1, from);

                if (next >= 0) {
                    pendingSong = next;

                    if (savefile->settings.volume > 0) {
                        clearText();
                        showModeText();
                        showStats(showingStats, totalTime, ppsStr, false);
                        showNowPlaying(next);
                    }
                } else {
                    // All songs disabled — stop music on unpause
                    pendingSong = -2;
                }
            }
        }

        if (key_hit(k.cancel) && confirmHoldAction < 0) {
            sfx(SFX_MENUCONFIRM);
            if (onStates) {
                onStates = false;
                selection = 0;
                showStats(showingStats, totalTime, ppsStr, false);
            } else {
                clearText();
                paused = false;
                break;
            }
        }

        if (key_hit(k.up) && confirmHoldAction < 0) {
            if (selection == 0)
                selection = maxSelection - 1;
            else
                selection--;
            sfx(SFX_MENUMOVE);
        }

        if (key_hit(k.down) && confirmHoldAction < 0) {
            if (selection == maxSelection - 1)
                selection = 0;
            else
                selection++;
            sfx(SFX_MENUMOVE);
        }

        if (key_hit(k.special1) && !onStates && !savefile->settings.journey &&
            confirmHoldAction < 0) {
            sfx(SFX_MENUCONFIRM);
            showingStats = !showingStats;
            clearSmallText();
            showStats(showingStats, totalTime, ppsStr, false);
        }

        if (!onStates && savefile->settings.journey) {
            if (key_is_down(k.special1)) {
                saveHoldTimer++;
                if (saveHoldTimer == holdThreshold) {
                    sfx(SFX_MENUCONFIRM);
                    saveToProfile(
                        &savefile->settings
                             .profiles[savefile->settings.selectedProfile],
                        &savefile->settings);
                    savefile->settings.journey = false;
                    if (previousSettings != nullptr)
                        *previousSettings = savefile->settings;
                    saveSavefile();
                    naprint(
                        "Graphics Saved!",
                        ((savefile->settings.aspectRatio) ? (240 - 13) : 240) -
                            getVariableWidth("Graphics Saved!"),
                        18 * 8);
                }
            } else if (saveHoldTimer > 0) {
                if (saveHoldTimer < holdThreshold) {
                    sfx(SFX_MENUCONFIRM);
                    showingStats = !showingStats;
                    clearSmallText();
                    showStats(showingStats, totalTime, ppsStr, false);
                }
                saveHoldTimer = 0;
            }

            if (key_is_down(k.special2)) {
                resetHoldTimer++;
                if (resetHoldTimer == holdThreshold) {
                    sfx(SFX_MENUCONFIRM);
                    setDefaultGraphics(savefile, 0);
                    setSkin();
                    setPalette();
                    drawFrameBackgrounds();
                    setClearEffect();
                    naprint(
                        "Graphics Reset!",
                        ((savefile->settings.aspectRatio) ? (240 - 13) : 240) -
                            getVariableWidth("Graphics Reset!"),
                        18 * 8);
                }
            } else {
                resetHoldTimer = 0;
            }
        }

        if (!shown && !onStates) {
            shown = true;
            showStats(showingStats, totalTime, ppsStr, false);
        }

        showSprites(128);
    }

    clearTilemap(27);
    clearTilemap(26);
    buildBG(3, 0, 27, 0, 1, true);

    enableBlend(prevBld);

    for (int i = 0; i < MAX_WORD_SPRITES; i++)
        wordSprites[i].hide();

    for (int i = 0; i < 2; i++)
        sprite_hide(cursorSprites[i]);

    drawFrame(0);
    if (game->zoneTimer)
        frameSnow(0);

    showBackground(0);
    resetSmallText();
    clearText();
    setSmallTextArea(110, 3, 7, 9, 10);
    showText();

    if (count && savefile->settings.pauseCountdown && !preventLayeredCountdowns)
        countdown();

    if (!preventLayeredCountdowns) {
        if (pendingSong >= 0)
            playSong(currentMenu, pendingSong);
        else if (pendingSong == -2)
            stopMusic();
        else
            resumeSong();
    }

    return 0;
}

// dir 1 is ascending, -1 is descending
int saveRecord(EntryBoard& board, int value, int dir) {

    for (int i = 0; i < 5; i++) {
        if (value * dir > board.entries[i].value * dir &&
            !(dir == 1 && board.entries[i].value == 0))
            continue;

        for (int j = 3; j >= i; j--)
            board.entries[j + 1] = board.entries[j];

        std::string name = nameInput(i);

        for (int j = 0; j < 8; j++)
            savefile->latestName[j] = ' ';

        for (int j = 0; j < name.size(); j++)
            savefile->latestName[j] = name.at(j);

        board.entries[i].value = value;

        board.entries[i].pro = proMode;

        strncpy(board.entries[i].name, name.c_str(), 9);

        return i;
    }

    return -1;
}

int onRecord() {
    int place = -1;

    if (replaying || game->pawn.big)
        return place;

    int subMode = game->subMode;

    if (game->gameMode == MARATHON) {
        if (subMode == 0) {
            place =
                saveRecord(savefile->boards.marathon[mode], game->score, -1);
        } else if (subMode == 1) {
            place = saveRecord(savefile->boards.zone[mode], game->score, -1);
        } else if (subMode == 2) {
            place = saveRecord(savefile->boards.tower[mode], game->score, -1);
        } else if (subMode == 3) {
            place = saveRecord(savefile->boards.cascade[mode], game->score, -1);
        }
    } else if (game->gameMode == SPRINT && game->won == 1) {
        if (subMode == 0) {
            place = saveRecord(savefile->boards.sprint[mode], gameSeconds, 1);
        } else {
            place =
                saveRecord(savefile->boards.sprintAttack[mode], gameSeconds, 1);
        }
    } else if (game->gameMode == DIG && game->won == 1) {
        if (subMode == 0) {
            place = saveRecord(savefile->boards.dig[mode], gameSeconds, 1);
        } else {
            place = saveRecord(savefile->boards.digEfficiency[mode],
                               game->pieceCounter, 1);
        }
    } else if (game->gameMode == ULTRA) {
        place = saveRecord(savefile->boards.ultra[mode], game->score, -1);

    } else if (game->gameMode == BLITZ) {
        place = saveRecord(savefile->boards.blitz[0], game->score, -1);

    } else if (game->gameMode == COMBO) {
        place =
            saveRecord(savefile->boards.combo, game->statTracker.maxCombo, -1);

    } else if (game->gameMode == SURVIVAL) {
        place = saveRecord(savefile->boards.survival[mode], gameSeconds, -1);

    } else if (game->gameMode == CLASSIC) {
        place = saveRecord(savefile->boards.classic[subMode], game->score, -1);

    } else if (game->gameMode == MASTER) {
        int grade = game->grade + game->coolCount + (int)game->creditGrade;

        if (grade > 32)
            grade = 32;

        EntryBoard& board = savefile->boards.master[subMode];

        for (int i = 0; i < 5; i++) {
            if (grade < board.entries[i].grade ||
                (grade == board.entries[i].grade &&
                 (gameSeconds > board.entries[i].value &&
                  board.entries[i].value != 0)))
                continue;

            for (int j = 3; j >= i; j--)
                board.entries[j + 1] = board.entries[j];

            std::string name = nameInput(i);

            for (int j = 0; j < 8; j++)
                savefile->latestName[j] = ' ';

            for (int j = 0; j < name.size(); j++)
                savefile->latestName[j] = name.at(j);

            board.entries[i].value = gameSeconds;
            board.entries[i].grade = grade;

            strncpy(board.entries[i].name, name.c_str(), 9);

            place = i;
            break;
        }
    } else if (game->gameMode == DEATH) {
        int grade = (game->level / 100) - game->regretCount;

        if (grade > 13)
            grade = 13;

        EntryBoard& board = savefile->boards.death[subMode];

        for (int i = 0; i < 5; i++) {
            if (grade < board.entries[i].grade ||
                (grade == board.entries[i].grade &&
                 (gameSeconds > board.entries[i].value &&
                  board.entries[i].value != 0)))
                continue;

            for (int j = 3; j >= i; j--)
                board.entries[j + 1] = board.entries[j];

            std::string name = nameInput(i);

            for (int j = 0; j < 8; j++)
                savefile->latestName[j] = ' ';

            for (int j = 0; j < name.size(); j++)
                savefile->latestName[j] = name.at(j);

            board.entries[i].value = gameSeconds;
            board.entries[i].grade = grade;

            strncpy(board.entries[i].name, name.c_str(), 9);

            place = i;
            break;
        }
    }

    if (place >= 0) {
        saveSavefile();
        framesSinceLastSave = 0;
    }

    return place;
}

void GameScene::showModeText() {
    const int width = (savefile->settings.aspectRatio == 0) ? 240 : 227;

    if (game->gameMode > 0 && game->gameMode <= 13) {
        int counter = 1;
        std::string str;
        std::string str2;

#ifndef MULTIBOOT
        str = modeStrings[game->gameMode - 1];

        if (game->gameMode == TRAINING)
            str = "Training";

        naprintColor(str, width - 4 - getVariableWidth(str), counter * 8, 14);
        counter++;

        str = "";
        if (game->gameMode == TRAINING) {
            if (game->trainingMode)
                str = "Finesse";
        } else {
            if (game->gameMode == BATTLE && mode >= 0)
                str = modeOptionStrings[game->gameMode - 1][mode - 1];
            else if (game->gameMode != CLASSIC && game->gameMode != MASTER)
                str = modeOptionStrings[game->gameMode - 1][mode];
            else if (game->gameMode == MASTER)
                str = modeOptionStrings[game->gameMode - 1][game->subMode];
            else
                str = modeOptionStrings[game->gameMode - 1][0];
        }

        if (!str.empty()) {
            naprintColor(str, width - 4 - getVariableWidth(str), counter * 8,
                         14);
            counter++;
        }

        str = "";
        str2 = "";
        if (game->subMode) {
            switch (game->gameMode) {
            case MARATHON:
                if (game->subMode == 3)
                    str = "Cascade";
                else if (game->subMode == 2)
                    str = "Tower";
                else
                    str = "Zone";
                break;
            case SPRINT:
                str = "Attack";
                break;
            case DIG:
                str = "Efficiency";
                break;
            case ZEN:
                if (game->subMode == 2)
                    str = "Cascade";
                else
                    str = "Tower";
                break;
            case CLASSIC:
                str = "B-Type";
                str2 = std::to_string(initialLevel) + "-" +
                       std::to_string(game->bTypeHeight);
                break;
            }
        } else {
            if (game->gameMode == CLASSIC)
                str = "A-Type";
        }

        if (!str.empty()) {
            naprintColor(str, width - 4 - getVariableWidth(str), counter * 8,
                         14);
            counter++;
        }
        if (!str2.empty()) {
            naprintColor(str2, width - 4 - getVariableWidth(str2), counter * 8,
                         14);
            counter++;
        }

        if (bigMode) {
            naprintColor("BIG", width - 4 - getVariableWidth("BIG"),
                         counter * 8, 14);
            counter++;
        }
#endif

        int mode = game->gameMode;

        if (game->rotationSystem != SRS) {
            if (mode != CLASSIC &&
                !((mode == MASTER || mode == DEATH) && game->subMode)) {
                std::string text;

                switch (game->rotationSystem) {
                case BlockEngine::NRS:
                    text = "NRS";
                    break;
                case BlockEngine::ARS:
                    text = "ARS";
                    break;
                case BlockEngine::BARS:
                    text = "BARS";
                    break;
                case BlockEngine::SDRS:
                    text = "SDRS";
                    break;
                default:
                    text = "SRS";
                    break;
                }

                naprintColor(text, width - 4 - getVariableWidth(text),
                             counter * 8, 14);
                counter++;
            }
        }

        if (game->randomizer != BAG_7) {
            if (mode != CLASSIC &&
                !((mode == MASTER || mode == DEATH) && game->subMode)) {
                std::string text;

                switch (game->randomizer) {
                case BlockEngine::BAG_7:
                    text = "7-BAG";
                    break;
                case BlockEngine::BAG_35:
                    text = "35-BAG";
                    break;
                default:
                    text = "RANDOM";
                    break;
                }

                naprintColor(text, width - 4 - getVariableWidth(text),
                             counter * 8, 14);
                counter++;
            }
        }
    }
}

Replay* test = nullptr;
// ReplayHeader test;

void saveReplay() {
    if (replaying)
        return;

    const int len = game->replay.size();
    if (len >= REPLAY_MAX_MOVES) {
        log("replay size too big (" + std::to_string(len) + ")");
        return;
    }

    if (!game->replayElligible) {
        log("replay not elligible");
        return;
    }

    if (test != nullptr)
        delete test;

    test = new Replay();

    test->header.tag = savefile->newGame;
    test->header.duration = game->timer;
    test->header.seed = game->initSeed;

    test->header.options = *previousGameOptions;

    auto it = game->replay.begin();

    int bytePos = 0;
    int prevTimer = 0;
    int movesWritten = 0;
    for (int i = 0; i < len; i++) {
        Timestamp move = *it;
        int delta = move.timer - prevTimer;
        prevTimer = move.timer;

        // action: [moveType:4][dir:1][unused:3]
        uint8_t action =
            (uint8_t)(((move.move & 0xF) << 1) | (move.dir ? 1 : 0));

        if (delta < 255) {
            if (bytePos + 2 > (int)sizeof(test->rawMoves))
                break;
            test->rawMoves[bytePos++] = (uint8_t)delta;
            test->rawMoves[bytePos++] = action;
            movesWritten++;
        } else {
            if (bytePos + 4 > (int)sizeof(test->rawMoves))
                break;
            test->rawMoves[bytePos++] = 255; // escape byte
            test->rawMoves[bytePos++] = (uint8_t)(delta & 0xFF);
            test->rawMoves[bytePos++] = (uint8_t)((delta >> 8) & 0xFF);
            test->rawMoves[bytePos++] = action;
            movesWritten++;
        }

        ++it;
    }

    test->header.length = movesWritten;
    test->header.rawLength = bytePos;

    // Snapshot end-game stats for replay interruption
    test->header.endScore = game->score;
    test->header.endLinesCleared = game->linesCleared;
    test->header.endLevel = game->level;
    test->header.endWon = game->won;
    test->header.endLost = game->lost;
    test->header.endGrade = game->grade;
    test->header.endCoolCount = game->coolCount;
    test->header.endCreditGrade = (int)(game->creditGrade * 100.0f);
    test->header.endPieceCounter = game->pieceCounter;
    test->header.endLinesSent = game->linesSent;
    test->header.endRegretCount = game->regretCount;
    test->header.endTowerScrollCount = game->towerScrollCount;
    test->header.endStackHeight = game->stackHeight;

    for (int i = 0; i < 4; i++)
        test->header.endClears[i] = game->statTracker.clears[i];
    test->header.endTSpins = game->statTracker.tspins;
    test->header.endPerfectClears = game->statTracker.perfectClears;
    test->header.endMaxStreak = game->statTracker.maxStreak;
    test->header.endMaxCombo = game->statTracker.maxCombo;
    test->header.endHolds = game->statTracker.holds;
    test->header.endMaxZonedLines = game->statTracker.maxZonedLines;
    test->header.endSecretGrade = game->statTracker.secretGrade;
    test->header.endInGameTimer = game->inGameTimer;

#ifdef PC
#ifdef __APPLE__
    std::string filePath = getSavefilePath();
    filePath.replace(filePath.find("Apotris.sav"), 11, "replay.sav");
    std::ofstream output(filePath, std::ios::binary | std::ios::out);
#else
    std::ofstream output("replay.sav", std::ios::binary | std::ios::out);
#endif

    char* src = (char*)test;
    output.write(src, sizeof(Replay));

    if (!output) {
        log("Error when trying to write replay.");
        return;
    } else {
        log("Wrote save to file.");
    }

    output.close();
#endif
}

ReplayHeader* currentReplayHeader = nullptr;
uint8_t* replayBytes = nullptr;
int replayCount = 0;

void injectReplayEndStats() {
    if (currentReplayHeader == nullptr || game == nullptr)
        return;

    game->score = currentReplayHeader->endScore;
    game->linesCleared = currentReplayHeader->endLinesCleared;
    game->level = currentReplayHeader->endLevel;
    game->won = currentReplayHeader->endWon;
    game->lost = currentReplayHeader->endLost;
    game->grade = currentReplayHeader->endGrade;
    game->coolCount = currentReplayHeader->endCoolCount;
    game->creditGrade = (float)currentReplayHeader->endCreditGrade / 100.0f;
    game->pieceCounter = currentReplayHeader->endPieceCounter;
    game->linesSent = currentReplayHeader->endLinesSent;
    game->regretCount = currentReplayHeader->endRegretCount;
    game->towerScrollCount = currentReplayHeader->endTowerScrollCount;
    game->stackHeight = currentReplayHeader->endStackHeight;

    for (int i = 0; i < 4; i++)
        game->statTracker.clears[i] = currentReplayHeader->endClears[i];
    game->statTracker.tspins = currentReplayHeader->endTSpins;
    game->statTracker.perfectClears = currentReplayHeader->endPerfectClears;
    game->statTracker.maxStreak = currentReplayHeader->endMaxStreak;
    game->statTracker.maxCombo = currentReplayHeader->endMaxCombo;
    game->statTracker.holds = currentReplayHeader->endHolds;
    game->statTracker.maxZonedLines = currentReplayHeader->endMaxZonedLines;
    game->statTracker.secretGrade = currentReplayHeader->endSecretGrade;

    // Set global game time from stored duration
    gameSeconds = (int)currentReplayHeader->duration;
    game->inGameTimer = currentReplayHeader->endInGameTimer;
}

void loadReplay() {
    replayBytes = nullptr;
    replayCount = 0;

    if (test != nullptr) {
        if (currentReplayHeader == nullptr)
            currentReplayHeader = new ReplayHeader();

        memcpy32_fast(currentReplayHeader, &test->header,
                      sizeof(ReplayHeader) / 4);

        replayBytes = test->rawMoves;
        replayCount = (int)test->header.length;
        replayIndex = 0;
        replayBytePos = 0;
        replayFrameTimer = 0;
        replaying = true;
        startGame(currentReplayHeader->options, currentReplayHeader->seed);

        gameLoop();
        return;
    }

#ifdef PC

#ifdef __APPLE__
    std::string filePath = getSavefilePath();
    filePath.replace(filePath.find("Apotris.sav"), 11, "replay.sav");
    std::ifstream input(filePath, std::ios::binary | std::ios::in);
#else
    std::ifstream input("replay.sav", std::ios::binary | std::ios::in);
#endif

    Replay rep;
    char* dst = (char*)&rep;

    log(std::to_string(sizeof(Replay)));

    input.read(dst, sizeof(Replay));

    input.close();

    if (!input) {
        log("Error when trying to load replay.");
        return;
    } else {
        log("Found replay file.");
    }

    if (rep.header.tag != savefile->newGame) {
        log("Invalid replay (old version, tag: " +
            std::to_string(rep.header.tag) + " ).");
        return;
    } else {
        log("Correct version number.");
    }

    if (rep.header.length > REPLAY_MAX_MOVES) {
        log("Invalid replay (too large).");
        return;
    }

    log("Attemping to load moves...");
    replayBytes = rep.rawMoves;
    replayCount = (int)rep.header.length;
    log("Loaded moves.");

    if (currentReplayHeader == nullptr)
        currentReplayHeader = new ReplayHeader();

    memcpy32_fast(currentReplayHeader, &rep.header, sizeof(ReplayHeader) / 4);

    replayIndex = 0;
    replaying = true;

    log("trying to start game...");
    startGame(currentReplayHeader->options, currentReplayHeader->seed);

    gameLoop();

#endif
}

void qrScene() {

    const std::string base = "https://knewjade.github.io/fumen-for-mobile/#?d=";

    const std::string fumen = game->getFumen();

    log(base + fumen);

    // const std::string link = base + fumen;

    // enum qrcodegen_Ecc errCorLvl = qrcodegen_Ecc_LOW;  // Error correction
    // level

    // const char *text = link.c_str();

    // // Make and print the QR Code symbol
    // uint8_t *qrcode = new uint8_t[qrcodegen_BUFFER_LEN_MAX];
    // uint8_t *tempBuffer = new uint8_t[qrcodegen_BUFFER_LEN_MAX];
    // bool ok = qrcodegen_encodeText(text, tempBuffer, qrcode, errCorLvl,
    //     qrcodegen_VERSION_MIN, qrcodegen_VERSION_MAX, qrcodegen_Mask_AUTO,
    //     true);

    // if(ok){
    //     int size = qrcodegen_getSize(qrcode);

    //     log(size);
    //     int border = 4;

    //     clearSprites(128);

    //     OBJ_ATTR * qr = &obj_buffer[0];
    //     const int qrTile = 300;

    //     clearSpriteTiles(qrTile, 8, 8);

    //     sprite_unhide(qr,0);
    //     sprite_set_attr(qr, ShapeSquare, 3, qrTile, 0, 0);
    //     sprite_enable_affine(qr, 8, true);
    //     sprite_set_size(qr, 1 << 7, 8);
    //     sprite_set_pos(qr,120 - (size + 4), 10);

    //     for (int y = -border; y < size + border; y++) {
    //         for (int x = -border; x < size + border; x++) {
    //             int color = (qrcodegen_getModule(qrcode, x, y) ? 6 : 7);

    //             int i = y + border;
    //             int j = x + border;
    //             setSpritePixel(qrTile, j / 8, i / 8, 8, j % 8, i % 8, color);
    //         }
    //     }

    //     showSprites(128);

    //     while(closed()){
    //         vsync();
    //         key_poll();

    //         if(key_hit(KEY_FULL)){

    //             break;
    //         }
    //     }
    // }

    // delete [] qrcode;
    // delete [] tempBuffer;
}

void addGameStats() {
    savefile->stats.timePlayed += frameCounter;
    frameCounter = 1;

    savefile->stats.gameStats.add(game->statTracker);
    savefile->stats.totalLines += game->linesCleared;

    if (game->gameMode == MARATHON && game->level > savefile->stats.maxLevel)
        savefile->stats.maxLevel = game->level;

    if (game->gameMode == ZEN) {
        if (game->subMode == 2) {
            int height =
                game->towerScrollCount + (game->lengthY - game->stackHeight);

            if (height > (int)savefile->boards.zenTower)
                savefile->boards.zenTower = height;
        } else if (game->subMode == 3) {
            savefile->boards.zenCascade = game->score;
            savefile->boards.zenLines =
                savefile->boards.zenLines + game->linesCleared;
        } else if (game->subMode == 1) {
            if (game->score > (int)savefile->boards.zenZone)
                savefile->boards.zenZone = game->score;
            savefile->boards.zenLines =
                savefile->boards.zenLines + game->linesCleared;
        } else {
            savefile->boards.zen = game->score;
            savefile->boards.zenLines =
                savefile->boards.zenLines + game->linesCleared;
        }

        saveSavefile();
    }
}

int GameScene::endScreenSetup() {
    resetSmallText();
    clearSmallText();
    setSmallTextArea(260, 1 + (savefile->settings.aspectRatio != 0), 1, 10, 20);

    for (auto& wordSprite : wordSprites)
        wordSprite.hide();

    if (!ended)
        endAnimation();

    enableBlend((1 << 6) + (0b11101 << 9) + (1 << 3));

    setTiles(27, 0, 32 * 32, tileBuild(34, false, false, 0));
    buildBG(3, 0, 27, 0, 0, true);
    clearTilemap(25);

    drawFrame(1);

    resetZonePalette();

    showScore();

    if (savefile->settings.skin >= 1000 && previousSettings != nullptr) {
        savefile->settings.skin = previousSettings->skin;
        savefile->settings.edges = previousSettings->edges;
        setSkin();
    }

    if (!replaying) {
        savefile->stats.gamesCompleted++;
        if (game->lost)
            savefile->stats.gamesLost++;

        addGameStats();
    }

    int record = -1;
    if (multiplayer) {
        progressBar(1);
    } else {
        record = onRecord();
    }

#ifdef GBA
    if (multiplayer &&
        !multiplayerLink->stateMachine.IsInState<MultiplayerStates::Lost>())
#endif
        showEndScreenOptions();

    return record;
}

void GameScene::showEndScreenOptions() {
    const int optionsHeight = 11;
    const bool finalMatch = multiplayer && multiplayerLink->matchComplete;

    int counter = 0;

    const int optionOffset = finalMatch ? 3 : 0;
    wordSprites[1].setText(finalMatch ? "View" : "Play");
    wordSprites[1].show(12 * 8, (optionsHeight + optionOffset + counter) * 8,
                         14);
    wordSprites[2].setText(finalMatch ? "Results" : "Again");
    wordSprites[2].show(
        14 * 8, (optionsHeight + optionOffset + counter + 1) * 8, 14);
#ifndef MULTIBOOT
    if (!finalMatch)
        counter += 3;
#endif

    if (!multiplayer) {
        wordSprites[3].setText("Change");
        wordSprites[3].show(12 * 8, (optionsHeight + counter) * 8, 14);
        wordSprites[4].setText("Options");
        wordSprites[4].show(14 * 8, (optionsHeight + counter + 1) * 8, 14);

        counter += 3;
    } else {
        showReadyPlayers();
        showScoreboard();
    }
#ifndef MULTIBOOT
    if (!finalMatch) {
        wordSprites[5].setText("Main");
        wordSprites[5].show(12 * 8, (optionsHeight + counter) * 8, 14);
        wordSprites[6].setText("Menu");
        wordSprites[6].show(14 * 8, (optionsHeight + counter + 1) * 8, 14);
    }
#endif

    showModeText();
}

void GameScene::showReadyPlayers() {
    int readyCount = multiplayerLink->getReadyCount();
    unsigned int playerCount = multiplayerLink->playingPlayerCount;
    if (playerCount > 0) {
        std::string readyText =
            std::to_string(readyCount) + "/" + std::to_string(playerCount);

        wordSprites[8].setText(readyText);

        wordSprites[8].show(236 - wordSprites[8].width, 160 - 12 - 8, 14);
    }

    if (multiplayerLink->players[multiplayerLink->universal->currentPlayerId()]
            .ready &&
        !multiplayerLink->stateMachine.IsInState<MultiplayerStates::Lost>()) {
        wordSprites[7].setText("Ready!");
#if !defined(NO_DIAGNOSE) && !defined(MULTIBOOT)
        if (enableBot) {
            wordSprites[7].setText("bot Ready!");
        }
#endif
        wordSprites[7].show(236 - wordSprites[7].width, 160 - 12, 15);
    } else if (multiplayerLink->stateMachine
                   .IsInState<MultiplayerStates::PlayersAwaitReady>()) {
        wordSprites[7].setText("Not Ready");
#if !defined(NO_DIAGNOSE) && !defined(MULTIBOOT)
        if (enableBot) {
            wordSprites[7].setText("botNot Ready");
        }
#endif
        wordSprites[7].show(236 - wordSprites[7].width, 160 - 12, 15);
    } else if (multiplayerLink->stateMachine
                   .IsInState<MultiplayerStates::Playing>()) {
        wordSprites[7].setText("Starting!");
        wordSprites[7].show(236 - wordSprites[7].width, 160 - 12, 15);
    } else if (multiplayerLink->stateMachine
                   .IsInState<MultiplayerStates::Lost>()) {
        wordSprites[7].setText("Waiting...");
        wordSprites[7].show(236 - wordSprites[7].width, 160 - 12, 15);
    } else if (multiplayerLink->stateMachine
                   .IsInState<MultiplayerStates::Won>()) {
        wordSprites[7].setText("Won!");
        wordSprites[7].show(236 - wordSprites[7].width, 160 - 12, 15);
    } else if (multiplayerLink->stateMachine
                   .IsInState<MultiplayerStates::PlayersSeeding>()) {
        wordSprites[7].setText("Seeding...");
        wordSprites[7].show(236 - wordSprites[7].width, 160 - 12, 15);
    } else if (multiplayerLink->stateMachine
                   .IsInState<MultiplayerStates::PlayersPresent>()) {
        wordSprites[7].setText("Syncing...");
        wordSprites[7].show(236 - wordSprites[7].width, 160 - 12, 15);
    } else if (multiplayerLink->stateMachine
                   .IsInState<MultiplayerStates::Activated>()) {
        wordSprites[7].setText("Reconnecting...");
        wordSprites[7].show(236 - wordSprites[7].width, 160 - 12, 15);
    } else {
        wordSprites[7].setText("Connecting...");
        wordSprites[7].show(236 - wordSprites[7].width, 160 - 12, 15);
    }
}

void GameScene::showScoreboard() {
    std::string score = std::to_string(multiplayerLink->wins) + "W - " +
                        std::to_string(multiplayerLink->losses) + "L";
    if (multiplayerLink->targetWins > 0)
        score += " (" + std::to_string(multiplayerLink->targetWins) + ")";
    wordSprites[9].setText(score);
    wordSprites[9].show(4, 160 - 12, 14);
}

void MultBattleScene::showReadyPlayers() {
    int readyCount = multiplayerLink->getReadyCount();
    unsigned int playerCount = multiplayerLink->playingPlayerCount;
    if (playerCount > 0) {
        std::string readyText =
            std::to_string(readyCount) + "/" + std::to_string(playerCount);

        wordSprites[8]->setText(readyText);

        wordSprites[8]->show(236 - wordSprites[8]->width, 160 - 12 - 8, 14);
    }

#ifndef GBA
    const int startX = 3 * 8;
    const int endX = 30 - 4;

    int verticalOffset = 0;

    std::unordered_map<u8, std::string> playerNames =
        multiplayerLink->getPlayerNames();

    int counter = 0;
    for (const std::pair<const u8, std::string>& pair : playerNames) {
        const int y = startY + 8 * (1) * counter + verticalOffset;

        char buff[12];
        posprintf(buff, "%d. %s", counter + 1, (pair.second).c_str());

        labels[counter]->setText(buff);
        labels[counter]->show(startX, y, 15);

        std::string str =
            multiplayerLink->players[pair.first].ready ? "READY" : " ";

        const int x = (endX) * 8 - getVariableWidth(str);
        wordSprites[counter]->setText(str);
        wordSprites[counter]->show(x, y, 15);

        counter++;
    }
#endif
}

void MultBattleScene::hideReadyPlayers() {
    for (int i = 0; i < 5; i++) {
        labels[i]->hide();
        wordSprites[i]->hide();
    }

    wordSprites[7]->hide();
    wordSprites[8]->hide();
}
