...
void addGlow(Drop);
void addPlaceEffect(const Drop&);

void showBar(int, int, int, int, int);
void showAttackBar(const std::list<Garbage>&, int, int, int, int);
...
Position gameLayout[(int)GameElements::LENGTH] = {
    Position(0, 0),          // NONE
    Position(10 * 8, 0),     // FIELD (must be *8)
    Position(5 * 8, 10 * 8), // HOLD
    Position(173, 12),       // QUEUE
};

Position altLayout[(int)GameElements::LENGTH] = {
    Position(0, 0),           // NONE
    Position(7 * 8, -32),     // FIELD (must be *8)
    Position(7 * 8 - 3, -56), // HOLD
    Position(173 - 29, -32),  // QUEUE
};

INLINE int prep(int n);

#ifdef GBA
__attribute((section(".iwram.game")))
#endif
Game* game = nullptr;
...
static bool rainbowIncreasing = false;
...
void GameScene::checkSounds() {
    if (game->sounds.attackApplied)
        sfx(SFX_ATTACK_APPLIED);

    if (game->sounds.attackReceived)
        sfx(SFX_ATTACK_RECEIVED);
    ...
    if (game->sounds.clear) {
        ...
        if (game->previousClear.cascadeComplexity > 1)
            speed += game->previousClear.cascadeComplexity - 1;
        if (speed > 10)
            speed = 10;
        ...
        if (game->zoneTimer) {
            ...
            if ((game->gameMode == MARATHON && game->subMode == 2) ||
                (game->gameMode == ZEN && game->subMode == 2)) {
        ...
        if (savefile->settings.clearText == 2 && !clearTypeText.empty())
            floatingList.emplace_back(clearTypeText);
    ...
    if (game->sounds.cascade) {
        if (savefile->settings.clearText == 2) {
            std::string cascadeText =
                "cascade x" + std::to_string(game->sounds.cascade);
            floatingList.emplace_back(cascadeText);
        }

        game->sounds.cascade = 0;
    }
    ...
    if (!sectionText.empty() && savefile->settings.clearText == 2)
        floatingList.emplace_back(sectionText);

    if (game->sounds.disappear) {
        disappear();
    }

    Position pos = gameLayout[(int)GameElements::FIELD];
    ...
    if (game->sounds.zone == 1) {
        ...
        for (int layer = 0; layer < 2; layer++) {
            for (int i = 0; i < 20; i++) {
                setTile(25 + layer, pos.x / 8 - 1, pos.y / 8 + i, 0);
                setTile(25, pos.x / 8 + 10, pos.y / 8 + i, 0);
            }

            for (int i = 0; i < 12; i++) {
                setTile(25 + layer, pos.x / 8 - 1 + i, pos.y / 8 + 20, 0);
            }
        }
        ...
        aprintClearArea(pos.x / 8, pos.y / 8, 10, 20);
        resetZonePalette();
        ...
        for (int i = startY; i < game->lengthY && (i - 20 + offset) < 20; i++) {
            ...
            if (game->bitboard[i] == 0 &&
                !((i < game->lengthY - 1 && game->bitboard[i + 1] != 0) ||
                  game->bitboard[i - 1] != 0)) {
                for (int j = 0; j < 10; j++)
                    setTile(25, pos.x / 8 + j, pos.y / 8 + y, 0);
                continue;
            }
            ...
            for (int j = 0; j < game->lengthX; j++) {
                ...
                if (!b || game->lineClearArray[i] ||
                    ...
                    setTile(25, pos.x / 8 + j, pos.y / 8 + y,
                ...
                if (n != 8)
                    setTile(
                        25, pos.x / 8 + j, pos.y / 8 + y,
                        tileBuild(offset, false, false, n * !game->zoneTimer));
                else
                    setTile(25, pos.x / 8 + j, pos.y / 8 + y,
                            tileBuild(3, false, false, 0));
            }
        }
            ...
            if (savefile->settings.clearDirection == 0) {
                for (int j = 0; j < 5; j++) {
                    if (clearTimer < maxClearTimer - 10 + j * 2)
                        setTile(25, pos.x / 8 + j, pos.y / 8 + y, clearTile);
                }
                for (int j = 5; j < 10; j++) {
...
void showPawn() {
    ...
    uint16_t* b = BlockEngine::getShape(game->activePiece, game->pawn.rotation,
                                        game->rotationSystem);
    ...
    if (savefile->settings.smoothGravity &&
        game->lockTimer == game->maxLockTimer && game->speed > 0) {
        smoothOffset = (game->speedCounter - (int)game->speedCounter) * 8;
    }
    ...
    if (!game->pawn.big) {
        ...
        sprite_set_pos(
            pawnSprite,
            pos.x + game->pawn.x * 8 + push * savefile->settings.shake,
            pos.y + (game->pawn.y - game->boardOffset + peek) * 8 +
                smoothOffset + shake * savefile->settings.shake + peekShift);
    } else {
        ...
        sprite_set_pos(
            pawnSprite,
            pos.x + (game->pawn.x * 2) * 8 + push * savefile->settings.shake,
            pos.y + (game->pawn.y - game->boardOffset + peek) * 8 * 2 +
                smoothOffset * 2 + shake * savefile->settings.shake +
                peekShift);
    }
}
...
void showShadow() {
    ...
    if (!game->pawn.big) {
        ...
        sprite_set_pos(pawnShadow,
                       ...
                       pos.y + (game->lowest() - game->boardOffset + peek) * 8 +
        ...
        sprite_set_pos(
            pawnShadow,
            pos.x + (game->pawn.x * 2) * 8 + push * savefile->settings.shake,
            pos.y + (game->lowest() - game->boardOffset + peek) * 2 * 8 +
                shake * savefile->settings.shake + peekShift);
    }
}
...
void showHold() {
    ...
    Position pos = gameLayout[(int)GameElements::HOLD];
    ...
    if (skin < 7 || skin == 9 || skin == 10 || skin > 1000) {
        ...
        sprite_set_pos(
            holdSprite, pos.x + add * 3 + 3 + (push < 0) * push,
            pos.y - 3 * (game->held == PIECE_I && !(rs == ARS || rs == BARS)) +
                yoffset +
        ...
        sprite_set_pos(
            holdSprite, pos.x + add * 3 + 3 - 4 + (push < 0) * push,
            pos.y - 3 * (game->held == PIECE_I && !(rs == ARS || rs == BARS)) +
                yoffset +
...
void showQueue(bool offsetSpriteIndex) {
    ...
    Position pos = gameLayout[(int)GameElements::QUEUE];
    ...
    if (game->zoneTimer) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            sprite_set_attr(queueFrameSprites[i], ShapeSquare, 2,
                            512 + 16 + 16 * i, 8, 3);
            sprite_set_pos(queueFrameSprites[i], pos.x + (push > 0) * push,
                           pos.y + 32 * i - (i * 9 * (5 - maxQueue)));

            sprite_unhide(background[i], 0);
            sprite_set_attr(background[i], ShapeSquare, 2, 512 + 416 + i * 16,
                            8, 3);
            sprite_enable_blend(background[i]);
            sprite_set_pos(background[i], pos.x + (push > 0) * push,
                           pos.y + 32 * i);
        }
    } else {
...
#ifdef TE
    if (flashTimer) {
        for (auto& queueSprite : queueSprites)
            sprite_hide(queueSprite);
        return;
    }
#endif
...
bool GameScene::control() {
    ...
    if (key_hit(k.zone) &&
        ((game->gameMode == MARATHON && game->subMode == 1) ||
         (game->gameMode == ZEN && game->subMode == 1)) &&
        !game->zoneTimer) {
        game->activateZone(1);
    }
...
void GameScene::showText() {
    ...
    if (gm != BATTLE && gm != BLITZ && gm != ZEN &&
        ...
        wordSprites[7].show(9 * 8 - wordSprites[7].width, 18 * 8, 15);
    } else if (gm == ZEN && (game->subMode == 0 || game->subMode == 1)) {
...
void showPPS() {
    ...
    std::string str = std::to_string(fx2int(pps)) + ".";

    int fractional = pps & 0xff;
    for (int i = 0; i < 2; i++) {
        fractional *= 10;
        str += (char)('0' + (fractional >> 8));
        fractional &= 0xff;
    }
...
void gameLoop() {
    changeScene([]() { return new GameScene(); }, Transitions::FADE);
...
void GameScene::countdown() {
    ...
    while (closed() && timer++ < timerMax - 1) {
        ...
        if (!multiplayer && !replaying && !interrupted && !demo &&
            ...
            (timer < 2 * timerMax / 3) &&
            (key_hit(k.rotateCW) || key_hit(k.rotateCCW) ||
             key_hit(k.rotate180))) {
            interrupted = true;
            timer = timerMax - 30;
            sfx(SFX_GO);
        ...
        if (timer < timerMax / 3) {
            aprint("READY?", pos.x / 8 + 2, pos.y / 8 + 10);
            ...
            aprint("  GO  ", pos.x / 8 + 2, pos.y / 8 + 10);
            if (timer == 2 * timerMax / 3 && savefile->settings.announcer &&
                !interrupted)
                sfx(SFX_GO);
        }
#ifdef GBA
        if (multiplayerLink->active && !multiplayerLink->sync() &&
            multiplayer) {
            clearText();
            break;
        }
...
void screenShake() {
    ...
    enableLayerWindow(1, 0, pos.y + shake, 240, pos.y + 160 + shake, false);
    enableLayerWindow(2, 0, pos.y + shake, 240, pos.y + 160 + shake, false);

#if defined(TE) || defined(N3DS)
    // allows for partially obsured ghost piece during peek
    setSpriteMaskRegion(pos.y + shake);
#endif
...
void drawFrame(int layer) {
    ...
    if (layer >= 0) {
        for (int i = 0; i < 20; i++) {
            for (int j = 0; j < 2; j++) {
                setTile(l, j * 11 + (pos.x) / 8 - 1, pos.y / 8 + i,
                        tileBuild(0x4, (j > 0), false, p));
            }
        }
        ...
        if (!game->zoneTimer) {
            setTile(l, (pos.x) / 8 - 1, pos.y / 8 + 20,
                    tileBuild(29, false, true, p)); // left bottom frame corner
            setTile(l, (pos.x) / 8 + 10, pos.y / 8 + 20,
                    tileBuild(29, true, true, p)); // right bottom frame corner

            // lower frame
            for (int i = 0; i < 10; i++)
                setTile(l, pos.x / 8 + i, pos.y / 8 + 20,
                        tileBuild(28, false, true, p));
        }
        ...
        for (int i = 0; i < 5; i++) {
            ...
            sprite_set_pos(frameSprite,
                           pos.x - 8 + push * savefile->settings.shake,
                           pos.y + i * 32 + frameShake);
        }
        ...
        for (int i = 0; i < 5; i++) {
            ...
            sprite_set_pos(frameSprite,
                           pos.x + 10 * 8 + push * savefile->settings.shake,
                           pos.y + i * 32 + frameShake);
        }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            sprite_set_pos(frameSprite,
                           pos.x - 8 + i * 32 + push * savefile->settings.shake,
                           pos.y + 20 * 8 + frameShake);
        }
    }
...
void drawGrid() {
...
#ifdef GBA
    setTiles(26, 31 * 32 + pos.x / 8, pos.y / 8 + 10,
             tileBuild(gridTile, false, false,
                       palOffset *
                           (savefile->settings.lightMode && !game->zoneTimer)));
    setTiles(26, 20 * 32 + pos.x / 8, pos.y / 8 + 10,
             tileBuild(gridTile, false, false,
                       palOffset *
                           (savefile->settings.lightMode && !game->zoneTimer)));
    ...
    if (savefile->settings.effects < 2) {
        ...
        for (int i = -2; i < 20; i++) {
            for (int j = 0; j < 10; j++) {
                if (i < peekAmount) {
                    ...
                    if (peekValue)
                        setTile(26, pos.x / 8 + j,
                                pos.y / 8 + i + ((wrap && i < 0) ? 32 : 0),
                                tileBuild(gridTile, false, false, 15));
                    else
                        setTile(26, pos.x / 8 + j, pos.y / 8 + (32 + i) & 31,
                                t);
                } else if (glow[i][j] == 0 || !savefile->settings.effects) {
                    setTile(26, pos.x / 8 + j, pos.y / 8 + i, t);
        ...
        for (int i = -2; i < 20; i++) {
            for (int j = 0; j < 10; j++) {
                if (i < peekAmount)
                    setTile(26, j + pos.x / 8, pos.y / 8 + i,
                            tileBuild(gridTile, false, false, 15));
                else
                    setTile(26, j + pos.x / 8, pos.y / 8 + i,
                            tileBuild(gridTile, false, false,
...
void progressBar(int layer) {
    ...
    if (game->gameMode != BATTLE) {
        showBar(current, max, pos.x / 8 + 10, 8, layer);

        if (layer >= 0) {
            setTile(l, pos.x / 8 - 1, pos.y / 8 + 20,
                    tileBuild(29, false, true, 8)); // left bottom frame corner
            setTile(
                l, pos.x / 8 + 10, pos.y / 8 + 20,
        ...
        showAttackBar(game->garbageQueue, 20, pos.x / 8 - 1, 8, layer);
...
void showBar(int current, int max, int x, int palette, int layer) {
    ...
    if (layer >= 0) {
        ...
        setTile(l, x, pos.y / 8, tileBuild(t, false, false, palette));
        ...
        setTile(l, x, pos.y / 8 + 19, tileBuild(t, false, true, palette));
        ...
        for (int i = 1; i < 19; i++) {
            ...
            setTile(l, x, pos.y / 8 + i, tileBuild(t, false, false, palette));
        }
    } else {
        TILE temp;
...
static inline u32 rowInteriorFill(int nibble) {
    if (nibble == 0)
        return 0x13000032;
    return (u32)(0x13000032 | (nibble << 20) | (nibble << 16) | (nibble << 12) |
                 (nibble << 8));
}

static inline u32 roundedShoulderFill(int nibble) {
    if (nibble == 0)
        return 0x13300332;
    return (u32)(0x13300332 | (nibble << 16) | (nibble << 12));
}

static inline int attackZoneNibble(int zone) {
    if (zone == 0)
        return 0;
    if (zone == 1)
        return 5;
...
void showAttackBar(const std::list<Garbage>& garbage, int max, int x,
                   int palette, int layer) {
    static std::vector<int> cachedIds;
    static std::vector<int> cachedAmounts;
    static bool lastWasEmpty = true;
    static int lastLayer = 0;

    bool flashOn = (attackFlashTimer < attackFlashMax / 2);
    int p = 8 + (layer < 0 ? 16 : 0);
    setPaletteColor(p, 4, 0x7fff, 1);
    setPaletteColor(p, 5, flashOn ? 0x7fff : 0x001f, 1);

    bool emptyNow = garbage.empty();

    // Skip rendering if nothing changed
    if (lastWasEmpty == emptyNow && layer == lastLayer) {
        if (!emptyNow) {
            if (garbage.size() == cachedIds.size()) {
                bool same = true;
                size_t i = 0;
                for (auto it = garbage.begin(); it != garbage.end();
                     ++it, ++i) {
                    if (it->id != cachedIds[i] ||
                        it->amount != cachedAmounts[i]) {
                        same = false;
                        break;
                    }
                }
    ...
    lastWasEmpty = emptyNow;
    lastLayer = layer;
    cachedIds.clear();
    cachedAmounts.clear();

    for (auto it : garbage) {
        cachedIds.push_back(it.id);
        cachedAmounts.push_back(it.amount);
    }
    ...
    if (garbage.empty()) {
        ...
        if (layer >= 0) {
            int baseY = pos.y / 8;
            setTile(l, x, baseY, tileBuild(6, false, false, palette));
            for (int i = 1; i < 19; i++)
                setTile(l, x, baseY + i, tileBuild(7, false, false, palette));
            setTile(l, x, baseY + 19, tileBuild(6, false, true, palette));
        } else {
            int base = 200 + (x > 9) * 20;
            setSpriteTile(base, 0, 0, 1, &sprite8tiles_bin[32 * 0]);
            for (int i = 1; i < 19; i++)
                setSpriteTile(base + (i / 4) * 4, 0, i % 4, 1,
                              &sprite8tiles_bin[32 * 1]);
            setSpriteTile(base + 16, 0, 3, 1, &sprite8tiles_bin[32 * 6]);
        }
        return;
    }
    ...
    int gp = 0;
    u8 zone = 1;
    for (auto it = garbage.begin(); it != garbage.end() && gp < 158;
         ++it, ++zone) {
        int pixelCount = it->amount * 8;
        if (gp + pixelCount > 158)
            pixelCount = 158 - gp;
        for (int k = 0; k < pixelCount; k++)
            pixelZone[gp++] = zone;
        auto next = it;
        ++next;
        if (next != garbage.end() && gp < 158)
            pixelZone[gp++] = 0;
    }
    for (int k = gp; k < 160; k++)
        pixelZone[k] = 0;

    int totalPixelFill = gp;
    int current = min((totalPixelFill * 20) / 158, 20);

    int segments = current;
    int pixels = (current * 158) / 20;
    int edge = pixels - segments * 8 + (segments != 0 ? 1 : 0);

    auto getZone = [&](int ti, int r) -> int {
        int idx = (19 - ti) * 8 + (7 - r);
        if (idx < 0 || idx >= 158)
            return 0;
        return pixelZone[idx];
    };

    auto isUniform = [&](int ti, int startRow, int endRow,
                         int* outZone) -> bool {
        int first = getZone(ti, startRow);
        *outZone = first;
        for (int r = startRow + 1; r <= endRow; r++) {
            if (getZone(ti, r) != first)
                return false;
        }
        return first != 0;
    };
    ...
    if (layer >= 0) {
        ...
        // Top tile (tile 0)
        int tileZone;
        bool uni = isUniform(0, 2, 7, &tileZone);
        ...
        if (uni) {
            if (segments == 19 && edge != 0)
                ...
                t = 8;
        } else if (segments == 19 && edge != 0)
            t = 10;
        else if (segments >= 20)
            t = 8;

        setTile(l, x, baseY, tileBuild(t, false, false, palette));
        if (!uni || tileZone == 1) {
            int shoulderNibble = attackZoneNibble(getZone(0, 2));
            setTileRow(0, t, 1, roundedShoulderFill(shoulderNibble));
            for (int r = 2; r < 8; r++) {
                int nibble = attackZoneNibble(getZone(0, r));
                setTileRow(0, t, r, rowInteriorFill(nibble));
            }
        }

        // Bottom tile (tile 19)
        uni = isUniform(19, 0, 6, &tileZone);
        ...
        setTile(l, x, baseY + 19, tileBuild(t, false, true, palette));
        if (!uni || tileZone == 1) {
            for (int r = 0; r < 7; r++) {
                int nibble = attackZoneNibble(getZone(19, r));
                int sourceRow = 7 - r;
                u32 row = r == 6 ? roundedShoulderFill(nibble)
                                 : rowInteriorFill(nibble);
                setTileRow(0, t, sourceRow, row);
            }
        }

        // Middle tiles (1..18)
        for (int i = 1; i < 19; i++) {
            uni = isUniform(i, 0, 7, &tileZone);
            ...
            setTile(l, x, baseY + i, tileBuild(t, false, false, palette));
            if (!uni || tileZone == 1) {
                for (int r = 0; r < 8; r++) {
                    int z = getZone(i, r);
                    if (z == 0)
                        setTileRow(0, t, r, rowInteriorFill(0));
                    else if (z == 1)
                        setTileRow(0, t, r, rowInteriorFill(5));
                }
            }
        }
    } else {
        ...
        // Top tile (tile 0)
        int spriteZone;
        bool uni = isUniform(0, 2, 7, &spriteZone);

        if (uni) {
            if (segments == 19 && edge != 0)
                memcpy32_fast(&temp, &sprite8tiles_bin[32 * 4], 8);
            else if (segments >= 20)
                memcpy32_fast(&temp, &sprite8tiles_bin[32 * 2], 8);
            else
                memcpy32_fast(&temp, &sprite8tiles_bin[32 * 0], 8);
        } else if (segments == 19 && edge != 0)
            memcpy32_fast(&temp, &sprite8tiles_bin[32 * 4], 8);
        else if (segments >= 20)
            memcpy32_fast(&temp, &sprite8tiles_bin[32 * 2], 8);
        ...
        if (!uni || spriteZone == 1) {
            int shoulderNibble = attackZoneNibble(getZone(0, 2));
            temp.data[1] = roundedShoulderFill(shoulderNibble);
            for (int r = 2; r < 8; r++) {
                int nibble = attackZoneNibble(getZone(0, r));
                temp.data[r] = rowInteriorFill(nibble);
            }
        }
        setSpriteTile(base, 0, 0, 1, &temp);

        // Bottom tile (tile 19)
        uni = isUniform(19, 0, 6, &spriteZone);

        if (uni) {
            if (segments == 0 && edge != 0)
                memcpy32_fast(&temp, &sprite8tiles_bin[32 * 8], 8);
            else if (segments == 0)
                memcpy32_fast(&temp, &sprite8tiles_bin[32 * 6], 8);
            else
                memcpy32_fast(&temp, &sprite8tiles_bin[32 * 7], 8);
        } else if (segments == 0 && edge != 0)
            memcpy32_fast(&temp, &sprite8tiles_bin[32 * 8], 8);
        else if (segments == 0)
            memcpy32_fast(&temp, &sprite8tiles_bin[32 * 6], 8);
        else
        ...
        if (!uni || spriteZone == 1) {
            for (int r = 0; r < 7; r++) {
                int nibble = attackZoneNibble(getZone(19, r));
                temp.data[r] = r == 6 ? roundedShoulderFill(nibble)
                                      : rowInteriorFill(nibble);
            }
        }
        setSpriteTile(base + 16, 0, 3, 1, &temp);

        // Middle tiles (1..18)
        for (int i = 1; i < 19; i++) {
            uni = isUniform(i, 0, 7, &spriteZone);

            if (uni) {
                memcpy32_fast(&temp, &sprite8tiles_bin[32 * 3], 8);
            } else if (19 - i > segments) {
                memcpy32_fast(&temp, &sprite8tiles_bin[32 * 1], 8);
            } else if (19 - i == segments) {
                memcpy32_fast(&temp, &sprite8tiles_bin[32 * 5], 8);
            } else {
                memcpy32_fast(&temp, &sprite8tiles_bin[32 * 3], 8);
            ...
            if (!uni || spriteZone == 1) {
                for (int r = 0; r < 8; r++) {
                    int z = getZone(i, r);
                    if (z == 0)
                        temp.data[r] = rowInteriorFill(0);
                    else if (z == 1)
                        temp.data[r] = rowInteriorFill(5);
                }
            }
            setSpriteTile(base + (i / 4) * 4, 0, i % 4, 1, &temp);
        }
    }
}
    ...
    for (int i = 0; it != placeEffectList.end(); i++) {
        ...
        case 1:
            ...
            if (game->rotationSystem == ARS && r == 2 ||
                (game->rotationSystem == SDRS ||
                 game->rotationSystem == BARS) &&
                    r == 0)
                yoffset += 8;
...
void addPlaceEffect(const BlockEngine::Drop& drop) {
    ...
    placeEffectList.emplace_back(pos.x / 8 + drop.x, pos.y / 8 + drop.y,
                                 drop.dx, drop.dy, drop.piece, drop.rotation,
...
void disappear() {
    ...
    while (!game->toDisappear.empty()) {
        ...
        setTile(25, x + pos.x / 8, pos.y / 8 + y - 20, 0);
...
void zoneFlash() {
    ...
    if (cond && (game->gameMode == MARATHON || game->gameMode == ZEN))
        gradient(true);
...
void frameSnow(int layer) {
    ...
    if (layer >= 0) {
        ...
        for (i = 0; i < 20 - ((20 * game->zoneTimer) / game->zoneStart); i++) {
            setTile(l, pos.x / 8 - 1, pos.y / 8 + i, 0);
            setTile(l, pos.x / 8 + 10, pos.y / 8 + i, 0);
        }

        if (i == 20)
            i = 19;

        setTile(l, pos.x / 8 - 1, pos.y / 8 + i,
                tileBuild(46, false, false, 0));
        setTile(l, pos.x / 8 + 10, pos.y / 8 + i,
                tileBuild(46, true, false, 0));

#if defined(TE) || defined(N3DS)

        setTile(l, pos.x / 8 + 10, pos.y / 8 + 20,
                tileBuild(29, true, true, 0)); // right bottom frame corner
        setTile(l, pos.x / 8 - 19, pos.y / 8 + 20,
                tileBuild(29, false, true, 0)); // left bottom frame corner

        for (i = 0; i < 10; i++)
            setTile(l, pos.x / 8 + i, pos.y / 8 + 20,
                    tileBuild(28, false, true, 0));
...
void showZoneText() {
    ...
    aprintColor(text, pos.x / 8 + 5 - text.size() / 2, pos.y / 8 + height, 2);
}
...
void GameScene::setupCredits() {
    ...
    for (auto& creditSprite : creditSprites) {
        creditSprite.setPriority(3);
    }
...
int replayIndex = 0;
int replayBytePos = 0;
int replayFrameTimer = 0;

void replayControl() {
    while (replayIndex < replayCount) {
        int savedBytePos = replayBytePos;
        int savedFrameTimer = replayFrameTimer;

        int delta = replayBytes[replayBytePos++];
        if (delta == 255) {
            delta = replayBytes[replayBytePos] |
                    (replayBytes[replayBytePos + 1] << 8);
            replayBytePos += 2;
        }
        replayFrameTimer += delta;

        if (replayFrameTimer > game->timer + game->eventTimer) {
            // Not time yet — rewind decoder state
            replayBytePos = savedBytePos;
            replayFrameTimer = savedFrameTimer;
            break;
        }

        uint8_t action = replayBytes[replayBytePos++];
        bool dir = action & 1;
        switch ((action >> 1) & 0xF) {
        case 0:
            game->keyLeft(dir);
...
void GameScene::update() {
    ...
    if (skipSong) {
        skipSong = false;
        if (savefile->settings.cycleSongs == 2) { // SHUFFLE
...
#ifndef MULTIBOOT
            enableBot = false;
            changeScene([]() { return new TitleScene(); }, Transitions::FADE);
#else
    ...
    if (clearTimer >= maxClearTimer || maxClearTimer <= 0) {
        ...
        rumblePatternLowPri(savefile->settings.rumbleStyle == 1
                                ? RUMBLE_SINGLE
    ...
    if ((game->gameMode == MARATHON && game->subMode && gameSeconds % 2 == 0) ||
        (game->gameMode == ZEN && game->subMode == 1 && gameSeconds % 2 == 0)) {
...
void showGoalLine() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        sprite_set_pos(
            sprite, pos.x + (3 * i) * 8 + push * savefile->settings.shake,
            pos.y + goalLineHeight * 8 - 1 + shake * savefile->settings.shake +
                peekShift + peek * 8);
    }
}
...
void showSpawn() {
    ...
    std::tuple<int, int> spawnPosition = game->getSpawnPosition();

    int x = std::get<0>(spawnPosition);
    int y = std::get<1>(spawnPosition) + add;
    ...
    if (!game->pawn.big) {
        ...
        sprite_enable_mosaic(sprite);
        sprite_set_pos(
            sprite, pos.x + x * 8 + push * savefile->settings.shake,
            pos.y + (y - game->boardOffset + peek) * 8 +
                shake * savefile->settings.shake + peekShift -
                8 * (game->rotationSystem == BARS && piece != PIECE_I ||
                     game->rotationSystem == SDRS));
    } else {
        sprite_set_attr(sprite, ShapeWide, 2, tile, 15, 2);
        sprite_enable_mosaic(sprite);
        ...
        sprite_set_pos(
            sprite, pos.x + (x * 2) * 8 + push * savefile->settings.shake,
            pos.y + (y - game->boardOffset + peek) * 8 * 2 +
                shake * savefile->settings.shake + peekShift -
                16 * (game->rotationSystem == BARS && piece != PIECE_I ||
                      game->rotationSystem == SDRS));
    }
}
