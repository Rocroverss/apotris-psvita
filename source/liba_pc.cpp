...
#include "sprite63tiles_bin.h"
#include "sprite64tiles_bin.h"
#include "sprite65tiles_bin.h"

#include <map>
#include <array>

#if defined(ANDROID)
static int offsetx = (SCREEN_WIDTH - 240) * 0.5;
static int offsety = (SCREEN_HEIGHT - 160) * 0.35;
#else
...
public:
    ...
    explicit ColorSplit(u16 c) {
        b = (((c) & 0x1f) << 3);
        g = (((c >> 5) & 0x1f) << (3));
        r = (((c >> 10) & 0x1f) << (3));
    }

    explicit ColorSplit(const u32* c) {
        b = (*c >> (16 + 3)) & 0x1f;
        g = (*c >> (8 + 3)) & 0x1f;
        r = (*c >> (0 + 3)) & 0x1f;
    }
...
#ifdef VITA
static int dirtyFirstLine = 0;
static int dirtyLastLine = SCREEN_HEIGHT;

void markRenderDirty() {
    dirtyFirstLine = 0;
    dirtyLastLine = SCREEN_HEIGHT;
}

static void markSpriteRowsDirty(const SpriteAttributes& sprite) {
    if (!sprite.enabled)
        return;

    // Affine sprites are positioned around a pivot. Use a conservative
    // vertical envelope so stale pixels can never remain in the cache.
    const int margin = sprite.affine ? sprite.sy : 1;
    const int first = std::max(0, sprite.y + offsety - margin);
    const int last = std::min(SCREEN_HEIGHT,
                              sprite.y + offsety + sprite.sy * 2 + margin);
    dirtyFirstLine = std::min(dirtyFirstLine, first);
    dirtyLastLine = std::max(dirtyLastLine, last);
}
#endif
...
void platformInit() {
    ...
    for (auto& tileset : tilesets) {
        tileset = TLN_CreateTileset(512, 8, 8, palettes[0], nullptr, nullptr);
    }
    ...
    for (auto& tilemap : tilemaps) {
        tilemap = TLN_CreateTilemap(64, 64, tiles, 0, tilesets[0]);
    }

    spriteBitmap = TLN_CreateBitmap(1024 * 8, 8, 8);

    TLN_SetBitmapPalette(spriteBitmap, palettes[16]); //  NECESSARY ! ! !
    spriteset = TLN_CreateSpriteset(spriteBitmap, spriteData, 128);
    TLN_SetSpritesetMapping1D(spriteset, true);
...
#ifdef VITA
    if (dirtyFirstLine < dirtyLastLine)
        TLN_UpdateFrameRegion(0, dirtyFirstLine, dirtyLastLine);
    dirtyFirstLine = SCREEN_HEIGHT;
    dirtyLastLine = 0;
#else
    TLN_UpdateFrame(0);
#endif
    ...
    for (int i = 4; i >= 1; i--) {
        for (int counter = 127; counter >= 0; counter--) {
            ...
            if (sprite->enabled && sprite->priority == i && sprite->id >= 0 &&
                sprite->id < 128) {
                spriteList[spriteCount++] = sprite->id;
            }
        }
    }

    static std::array<SpriteAttributes, 128> previous{};
    static std::array<int, 128> previousIds = [] {
        std::array<int, 128> ids{};
        ids.fill(-1);
        return ids;
    }();
    static std::array<bool, 128> previousBlend{};

    auto sameSprite = [](const SpriteAttributes& a,
                         const SpriteAttributes& b) {
        return a.enabled == b.enabled && a.tile_start == b.tile_start &&
               a.x == b.x && a.y == b.y && a.sx == b.sx && a.sy == b.sy &&
               a.affine == b.affine && a.scalex == b.scalex &&
               a.scaley == b.scaley && a.palette == b.palette &&
               a.priority == b.priority && a.flipx == b.flipx &&
               a.flipy == b.flipy && a.rotate == b.rotate &&
               a.angle == b.angle && a.blend == b.blend &&
               a.doubleSize == b.doubleSize;
    };

    for (int counter = 0; counter < spriteCount; counter++) {
        const int id = spriteList[counter];
        OBJ_ATTR* sprite = &obj_buffer[id];
        const bool effectiveBlend = blendSprites || sprite->blend;
        if (previousIds[counter] == id &&
            previousBlend[counter] == effectiveBlend &&
            sameSprite(previous[counter], *sprite)) {
            continue;
        }

#ifdef VITA
        if (previousIds[counter] != -1)
            markSpriteRowsDirty(previous[counter]);
        markSpriteRowsDirty(*sprite);
#endif

        spriteData[counter].x = sprite->tile_start * 64;
        spriteData[counter].y = 0;
        spriteData[counter].w = sprite->sx;
        spriteData[counter].h = sprite->sy;

        TLN_SetSpriteSet(counter, spriteset);
        TLN_SetSpritesetData(spriteset, counter, &spriteData[counter], nullptr,
                             0);
        ...
        previous[counter] = *sprite;
        previousIds[counter] = id;
        previousBlend[counter] = effectiveBlend;
    }

    for (int counter = spriteCount; counter < 128; counter++) {
        if (previousIds[counter] != -1) {
#ifdef VITA
            markSpriteRowsDirty(previous[counter]);
#endif
            TLN_DisableSprite(counter);
            previousIds[counter] = -1;
        }
    }
}
...
#ifdef VITA
    const bool changed = layerInfo[layer].x != x || layerInfo[layer].y != y;
#endif
    layerInfo[layer].x = x;
    layerInfo[layer].y = y;
...
void loadSpriteTiles(int index, const void* src, int lengthX, int lengthY) {
    ...
    uint8_t* base = TLN_GetBitmapPtr(spriteBitmap, 0, 0);

    for (int ii = 0; ii < count; ii++) {
        uint8_t* dst = base + (index + ii) * 64;
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 4; j++) {
                dst[i * 8 + j * 2] = source[ii * 32 + i * 4 + j] & 0xf;
                dst[i * 8 + j * 2 + 1] =
                    (source[ii * 32 + i * 4 + j] >> 4) & 0xf;
            }
        }
...
void loadSpriteTilesPartial(int index, const void* src, int tx, int ty,
    ...
    const int count = lengthX * lengthY;
    uint8_t* base = TLN_GetBitmapPtr(spriteBitmap, 0, 0);
    const int startTile = index + ty * rowLength + tx;

    for (int ii = 0; ii < count; ii++) {
        uint8_t* dst = base + (startTile + ii) * 64;
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 4; j++) {
                dst[i * 8 + j * 2] = source[ii * 32 + i * 4 + j] & 0xf;
                dst[i * 8 + j * 2 + 1] =
...
void clearSpriteTiles(int index, int lengthX, int lengthY) {
    uint8_t* base = TLN_GetBitmapPtr(spriteBitmap, 0, 0);
    memset(base + index * 64, 0, lengthX * lengthY * 64);
}
...
void clearSpriteTile(int index, int tx, int ty, int width) {
    uint8_t* base = TLN_GetBitmapPtr(spriteBitmap, 0, 0);
    memset(base + (index + ty * width + tx) * 64, 0, 64);
};
...
void onHBlank(int line) {
    ...
    auto c = ColorSplit(gradientTable[line]);
    TLN_SetBGColor(c.r, c.g, c.b);
}
...
u16 getPaletteColor(int palette, int index) {
    return rgb8to5((u32*)TLN_GetPaletteData(palettes[palette], index));
}

void savePalette(COLOR* dst) {
    for (auto& palette : palettes)
        for (int j = 0; j < 16; j++)
            *dst++ = rgb8to5((u32*)TLN_GetPaletteData(palette, j));
}
...
void showBattery() {
    ...
    OBJ_ATTR* sprite = &obj_buffer[127];

    if (savefile->settings.batteryDisplay == 0 || batteryPercentage == -1 ||
    ...
    char buff[8];
    snprintf(buff, 8, "%d%%", batteryPercentage);

    u8* src = (u8*)sprite63tiles_bin;

    if (charging) {
        src = (u8*)sprite65tiles_bin;
    } else if (batteryPercentage <= 15) {
        src = (u8*)sprite64tiles_bin;
    }

    loadSpriteTiles(1022, src, 1, 1);

    clearSpriteTiles(1020, 2, 1);
    aprintsSprite(buff, 4, 0, 1020);

    sprite_set_attr(sprite, ShapeWide, 1, 1020, 15, 0);
    sprite_set_pos(sprite, 21 * 8 + 1, 154);
...
void setSpriteMaskRegion(int shift) {
    ...
    TLN_SetSpritesMaskRegion(top, 0);
}
...
void setScreenOffset(int x, int y) {
    ...
    for (int i = 0; i < 4; i++) {
        setLayerScroll(i, layerInfo[i].x, layerInfo[i].y);
    }
...
void setUpPresentException(std::string error) {
    ...
    changeScene(
        [&error]() { return new MultiplayerExceptionScene(std::move(error)); });
}
