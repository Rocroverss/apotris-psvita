...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (std::string(argv[i]) == "--scenario") {
            if (i + 1 == argc) {
                std::fprintf(stderr, "--scenario requires a file path\n");
                return 2;
            }
            scenarioPath = argv[++i];
        }
    ...
    while (closed()) {
#ifdef PC
        if (scenarioConsumeRestartRequest())
            scenarioRestart();
#endif
        checkSceneSwitch();
        vsync();
        // This is here to be thread safe on PC
...
std::string nameInput(int place, bool* cancelled) {

    if (cancelled != nullptr)
        *cancelled = false;

    std::string result = place == -1 ? "         " : savefile->latestName;
    int cursor = 0;
...
#if defined(N3DS) || defined(ANDROID)
    resetSmallText();
    ...
    while (closed()) {
        ...
        if (place == -1) {
            aprint("Pick a unique room", 6, 5);
            aprint("to join/create", 7, 6);
        }

        if (place == -1) {
            aprint("Room pass: ", 11, nameHeight - 2);
        } else {
            aprint("Name: ", 11, nameHeight - 2);
        }
        ...
        // With a room code, cancel keeps its normal backspace behavior. Only
        // leave the room-entry menu when there is nothing left to delete.
        if (place == -1 && key_hit(k.cancel) &&
            result.find_first_not_of(' ') == std::string::npos) {
            if (cancelled != nullptr)
                *cancelled = true;
            sfx(SFX_MENUCANCEL);
            break;
        ...
        if (!onDone) {
            ...
            for (unsigned char character : consumeTextInput()) {
                if (character == '\n' || character == '\r') {
                    onDone = true;
                    stopTextInput();
                    sfx(SFX_MENUCONFIRM);
                ...
                if (character == '\b') {
                    if (cursor > 0 && (cursor < 7 || result[cursor] == ' ')) {
                        result[--cursor] = ' ';
                        sfx(SFX_MENUMOVE);
            ...
            if (!onDone && (key_hit(SDLK_RETURN) || key_hit(SDLK_ESCAPE))) {
                onDone = true;
                stopTextInput();
                sfx(SFX_MENUCONFIRM);
            }
        ...
        keyboard:
            ...
            if (key_hit(k.confirm) || key_hit(k.pause)) {
                if (place == -1 &&
                    result.find_first_not_of(' ') == std::string::npos) {
                    // Do not pass an all-space room ID to the WebSocket URL.
                    // The signaling server requires a non-empty room/action.
                    onDone = false;
                    sfx(SFX_MENUCANCEL);
                } else {
                    sfx(SFX_MENUCONFIRM);
...
void setLightMode() {
    if (savefile->settings.lightMode) {
        ...
        setPaletteColor(8, 5, 0x001f, 1);
        setPaletteColor(16 + 8, 5, 0x001f, 1);
...
void setPalette() {
    ...
    setPaletteColor(30, 14,
                    getPaletteColor(8, 2 + savefile->settings.lightMode), 1);
    setPaletteColor(31, 14, RGB15(31, 5, 5), 1);

#if !defined(GBA) && !defined(MULTIBOOT)
    for (int i = 0; i < 7; i++) {
        setPaletteColor(
            16 + 12, 6 + i,
            getPaletteColor(16 + i, 3 - savefile->settings.lightMode * 2), 1);
    }
    setPaletteColor(16 + 12, 13, getPaletteColor(16 + 7, 3), 1);
    setPaletteColor(16 + 12, 14, RGB15(27, 29, 31), 1);
#endif
}
    ...
    *previousGameOptions = options;
    ...
    if (options.mode == BlockEngine::ZEN) {
        if (game->subMode == 2) {
            ...
            game->score = savefile->boards.zenZone;
        } else {
...
void startBotGame(int seed) {
    ...
    botGame = new Game(BATTLE, seed & (SEND_SEED - 1), false);
...
#elif defined(SWITCH) || defined(MM) || defined(PORTMASTER) || defined(VITA)

    return keyToString[key];

#elif defined(WEB) || defined(PC) || defined(ANDROID)
