...
u8 BlockEngine::fumenQueue[5] = {PIECE_I, PIECE_L, PIECE_J, PIECE_T, 0};
...
void Pawn::setBlock(int system) {
    ...
    for (int r = 0; r < 4; r++) {
        u16* shape = getShape(current, r, system);
...
void Game::rotate(int dir, int on) {
    ...
    if ((clearLock || entryDelay || cascadeTimer)) {
        fromLockRotate = true;
        return;
    } else if (eventLock) {
        return;
...
void Game::hardDrop() {
    if (clearLock || eventLock || dropLockTimer || entryDelay || cascadeTimer)
        return;
...
void Game::update() {
    ...
    if (unstable.size()) {
        if (cascadeTimer++ >= CASCADE_DELAY || maxClearDelay == 1) {
            cascadeTimer = 0;
        } else {
            return;
        }
        ...
        if (unstable.size() == 0) {
            Drop d;
            d.startY = stackHeight - boardOffset;
            d.rawEndY = lengthY - 1;
            clear(d);
            calculateColumnHeights(0);
        }

        return;
    }

    if (cascadeComplexity > 1) {
        if (cascadeComplexity > statTracker.maxCascade)
            statTracker.maxCascade = cascadeComplexity;

        sounds.cascade = cascadeComplexity;
        cascadeComplexity = 0;
    }
    ...
    if (das == maxDas && !(left && right)) {
        if (arr >= 0 && --arrCounter <= 0 && !(gameMode == CLASSIC && down)) {
            ...
            for (int i = 0; i < 1 + (arr == 0);
                ...
                if (!zoneTimer && speed == 20 && gameMode != BLITZ) {
                    int counter = 0;
    ...
    if (!replayElligible || replay.size() > REPLAY_MAX_MOVES) {
        replay.clear();
...
void Game::place() {
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            blocks |= ((pawn.board[pawn.rotation][i][j] != 0) << j) << (4 * i);
        }
    }

    bool isCascade = (gameMode == MARATHON && subMode == 3) ||
                     (gameMode == ZEN && subMode == 3);
    if (isCascade)
        connections.emplace_back(pawn.x, pawn.y, blocks);
    ...
    if ((pawn.y + pawn.lowestBlock[pawn.rotation]) <= (lengthY / 2 - 1)) {
        ...
        else if (gameMode != ZEN && !(gameMode == MARATHON && subMode == 2)) {
            lost = 1;
            return;
        }
    }
...
int Game::clear(const Drop& drop) {
    ...
    int start = drop.startY + boardOffset;
    int end = drop.rawEndY + 1;
    ...
    const bool isCascade = (gameMode == MARATHON && subMode == 3) ||
                           (gameMode == ZEN && subMode == 3);
    ...
    } else if (clearCount > 4 && !isPerfectClear && !isTSpin) {
        // score >4 lines as groups of 4 (e.g. 6 = quad + double)
        int quads = clearCount / 4;
        int remainder = clearCount % 4;

        if (gameMode != CLASSIC) {
            add += quads * scoring[3][0] * prevLevel;
            attack = quads * scoring[3][2];
            if (remainder > 0) {
                add += scoring[remainder - 1][0] * prevLevel;
                attack += scoring[remainder - 1][2];
            }
        } else {
            add += quads * classicScoring[3] * (level + 1);
            if (remainder > 0)
                add += classicScoring[remainder - 1] * (level + 1);
        }

        isDifficult = true;
        statTracker.clears[3] += quads;
        if (remainder > 0)
            statTracker.clears[remainder - 1]++;
    }
    ...
    previousClear =
        Score(clearCount, add, comboCounter, isTSpin, isPerfectClear,
              isBackToBack, isDifficult, cascadeComplexity, drop);
...
void Game::fillQueue(int count) {
    for (int i = 0; i < count; i++) {
        if (randomizer == BAG_7) {
            ...
            if (proMode && queue.empty() && bagCounter == 1 && bag.size() == 7) {
                while (bag.array[n] == PIECE_S || bag.array[n] == PIECE_Z || bag.array[n] == PIECE_O) {
                    n = random() % bag.size();
                }
            }
...
void Game::hold(int dir) {
    ...
    if ((clearLock || entryDelay || cascadeTimer) && gameMode != CLASSIC) {
        // holding = dir;
        fromLockHold = true;
        return;
    } else if (dir == 0) {
...
uint16_t* BlockEngine::getShape(int n, int r, int rotationSystem) {
    uint16_t* result;
    result = new uint16_t[4 * 4];

    uint16_t* source;

    switch (rotationSystem) {
    case SRS:
        source = (uint16_t*)tetrominos[n][r];
        break;
    case NRS:
        source = (uint16_t*)classic[n][r];
        break;
    case ARS:
        source = (uint16_t*)ars[n][r];
        break;
    case BARS:
        source = (uint16_t*)bars[n][r];
        break;
    case SDRS:
        source = (uint16_t*)sdrs[n][r];
        break;
    default:
        source = (uint16_t*)tetrominos[n][r];
        break;
    ...
    memcpy32_fast(result, source, 8);
    return result;
}

void Game::lockCheck() {
    if (pawn.lowest == pawn.y && lockMoveCounter > 0 &&
        (rotationSystem == SRS || rotationSystem == BARS ||
         rotationSystem == SDRS)) {
        lockTimer = maxLockTimer;
        lockMoveCounter--;
    }
...
void Game::keyLeft(int dir) {
    ...
    if (clearLock || eventLock || entryDelay || cascadeTimer ||
        (gameMode == CLASSIC && down)) {
        left = dir;
        return;
    }
...
bool Game::cascade() {
    ...
    do {
        // resolve unstable
        auto it = unstable.begin();
        while (it != unstable.end()) {
            Connection* piece = *it;
            ...
            for (int i = 3; i >= 0; i--) {
                for (int j = 0; j < 4; j++) {
                    if (!((piece->blocks >> (i * 4 + j)) & 0b1))
                        continue;

                    if (y + i + 1 < lengthY)
                        board[y + i + 1][x + j] = board[y + i][x + j];
                    board[y + i][x + j] = 0;

                    if (y + i + 1 < lengthY)
                        bitboard[y + i + 1] |= 1 << (x + j);
                    bitboard[y + i] &= ~(1 << (x + j));
        ...
        for (auto& connection : connections) {
            ...
            bool stable = false;
            int checkedColumnsBits = 0;
            for (int i = 3; i >= 0; i--) {
                int currentLine = (connection.blocks >> 4 * i) & 0xf;

                // Only lowest block in each column can provide support.
                currentLine &= ~checkedColumnsBits;

                if (currentLine == 0) {
                    continue;
                }

                const int supportMask = connection.x >= 0
                                            ? currentLine << connection.x
                                            : currentLine >> -connection.x;
                if (connection.y + i >= lengthY - 1 ||
                    bitboard[connection.y + i + 1] & supportMask) {
                    stable = true;
                    break;
                }

                checkedColumnsBits |= currentLine;
            }

            if (!stable) {
                unstable.push_back(&connection);
            }
        }
    } while (CASCADE_INSTANT && unstable.size() && counter++ < 1000);
...
void Game::removeClearLock() {
    ...
    int lowestLine = linesToClear.front();
    ...
    if (!isCascade) {
        ...
        for (auto const& line : linesToClear) {
            memset32_fast(board[line], 0, 10);
            bitboard[line] = 0;
            if (disappearing)
                memset16(disappearTimers[line], 0, 10);
        }
    }
    ...
    if (!entryDelay && !zoneExit && !unstable.size()) {
        next();
    } else if (zoneExit) {
        zoneExit = false;
    }
...
void Game::fillReplayToLimit() {
    return;

    // Fill replay with synthetic entries until the delta-encoded
    // byte stream would overflow the REPLAY_BUF_SIZE-byte rawMoves buffer.
    // Fake moves share a constant distant timer so they never replay,
    // producing: 4 bytes for the bridge gap + 2 bytes each thereafter.

    const int bufCapacity = REPLAY_BUF_SIZE;
    const int current = (int)replay.size();

    // Estimate bytes for existing moves.
    int byteEstimate = 0;
    int prevTimer = 0;
    for (auto& m : replay) {
        int delta = m.timer - prevTimer;
        prevTimer = m.timer;
        byteEstimate += (delta < 255) ? 2 : 4;
    }

    // Bridge from last real move to the distant fake timer.
    int lastTimer = (current > 0) ? replay.back().timer : 0;
    int fakeTimer = timer + 1000000;
    int firstDelta = fakeTimer - lastTimer;
    int bridgeCost = (firstDelta < 255) ? 2 : 4;

    int remaining = bufCapacity - byteEstimate;
    if (remaining < bridgeCost)
        return;

    // 1 bridge entry + as many delta=0 entries as fit.
    int count = 1 + (remaining - bridgeCost) / 2;

    for (int i = 0; i < count; i++) {
        replay.emplace_back(fakeTimer, 0, false);
    }
}
...
std::vector<int> Game::getIncomingAttacks() const {
    std::vector<int> result;
    for (const auto& atck : garbageQueue)
        result.push_back(atck.amount);
    return result;
}
...
// source list holds lines to clear sorted from bottom to top
void Game::fixConnected(const std::list<int>& sourceList) {
    ...
    while (connection != connections.end()) {
        ...
        for (auto const& line : sourceList) {
            ...
            connection->blocks &= ~(0xf << localY * 4);

            if (connection->blocks == 0)
                break;
        }

        // check if connection needs to be split
        // (don't need to check 1 block connections)
        if (before != connection->blocks && countBits(connection->blocks) > 1) {
            for (int i = 3; i > 0; i--) {
                const int currentLine = (connection->blocks >> 4 * i) & 0xf;
                const int previousLine =
                    (connection->blocks >> 4 * (i - 1)) & 0xf;

                if (currentLine == 0)
                    continue;

                // if current and previous line don't have any bits in common
                // then they are disconnected and current needs to split
                if ((currentLine & previousLine) == 0) {
                    int mask = 0;
                    ...
                    const int newBlocks = mask & connection->blocks;

                    splits.push_back(Connection(x, y, newBlocks));

                    connection->blocks &= ~(newBlocks);

                    if (countBits(connection->blocks) <= 1)
                        break;
                }
            }
        }
...
Game::Game(int gm, int sd, bool big) {
    ...
    for (auto& disappearTimer : disappearTimers) {
        for (int j = 0; j < 10; j++)
            disappearTimer[j] = 0;
    }
}
...
void Game::setupPawnSpawn() {
    ...
    if (rotationSystem == ARS ||
        (rotationSystem == NRS && gameMode != CLASSIC) ||
        rotationSystem == SDRS) {
        pawn.y--;
        if (rotationSystem == NRS && pawn.current == PIECE_I &&
            gameMode != CLASSIC)
            pawn.y--;
    }
...
void Game::scrollTower() {
    ...
    if (stackHeight == lengthY) {
        score += 3500 * level;
        sounds.clear = 1;
        previousClear = Score(0, 0, -1, 0, 1, 0, 0, 1, Drop());
    }
...
void Game::calculateColumnHeights(int start) {
    // recalculate column heights
    int max = 0;
    for (int x = 0; x < lengthX; x++) {
        int found = -1;
        for (int y = lengthY - (columnHeights[x] - start) - 1; y < lengthY;
             y++) {
            if (board[y][x]) {
                found = y;
                break;
        ...
        if (max < columnHeights[x]) {
            max = columnHeights[x];
        }
    }
    stackHeight = lengthY - max;
}
