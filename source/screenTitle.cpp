...
#ifdef MULTIBOOT
__attribute((section(".iwram.word_sprites")))
#endif
...
#ifdef GBA
__attribute((section(".ewram.data")))
#endif
u16 backgroundArray[bgGridHeight][30];
u32 backgroundTileCache[32][30];
...
const std::list<std::string> gameOptions = {
    "Marathon",     "Sprint",  "Dig",    "Ultra", "Blitz", "Combo",
    "Survival",     "Classic", "Master", "Death", "Zen",
#if !defined(SWITCH) && !defined(VITA)
    "Multi Battle",
#endif
#ifdef GBA
//"Wireless Pak", "Wired Pak",
#endif
    "CPU Battle",   "Training"};
...
void fallingBlocks() {
...
#ifdef VITA
    // The falling field is decorative. Updating its tile layer at 10 Hz keeps
    // the motion while allowing the already-composed menu background and text
    // to remain cached for five out of every six 60 Hz presentation frames.
    static unsigned backgroundDrawDivider = 0;
    const bool updateBackgroundTiles = (++backgroundDrawDivider % 6) == 0;
#else
    const bool updateBackgroundTiles = true;
#endif
    ...
    static int cachedSkin = -1000;
    static bool cachedBigMode = false;
    if (cachedSkin != savefile->settings.skin || cachedBigMode != bigMode) {
        memset32(backgroundTileCache, -1, 32 * 30);
        cachedSkin = savefile->settings.skin;
        cachedBigMode = bigMode;
    }

    if (updateBackgroundTiles) {
        ...
        for (j = 0; j < 30; j++) {
            ...
            if (backgroundArray[i + offset][j]) {
                ...
                if (savefile->settings.skin >= 11)
                    tile = tileBuild(128 + GameInfo::connectedConversion[r],
                                     false, false, n);
                // *dest++ = 128 + GameInfo::connectedConversion[r] + ((n) <<
                // 12);
                else if (savefile->settings.skin < 7 ||
                         savefile->settings.skin > 8)
                    tile = tileBuild(1, false, false, n);
                // *dest++ = (1 + ((n) << 12));
                else {
                    tile = tileBuild(48 + n, false, false, n);
                    // *dest++ = (48 + n + ((n) << 12));
                }
            }
            if (backgroundTileCache[i][j] != tile) {
                setTile(25, j, i, tile);
                backgroundTileCache[i][j] = tile;
            }
        }
        // dest += 2;
      }
...
#ifdef GBA
void MultiplayerProtocolScene::init() {
    clearSprites(128);
    showSprites(128);
    ...
    clearTilemap(29);
    naprint("How are you connected?", 16, 56);

    wordSprites[0].setText("CABLE");
    wordSprites[0].show(32, 104, 14);

    wordSprites[1].setText("WIRELESS");
...
bool MultiplayerProtocolScene::control() {
...
void MultiplayerProtocolScene::draw() {
    fallingBlocks();
#ifdef N3DS
...
void MultiplayerProtocolScene::update() {
    canDraw = true;
    key_poll();
...
void MultiplayerProtocolScene::deinit() {
    clearText();
    clearSmallText();
    resetSmallText();
}
...
#ifndef NO_DIAGNOSE
    wordSprites[2].setText("DEBUG BUILD");
#endif
...
std::function<Scene*()> MainMenuScene::previousScene() {
    return [] { return new TitleScene(); };
}
...
void MainMenuScene::update() {
    if (!updateAvailable.empty() && !showingUpdateNotification) {
        wordSprites[10]->setText("Update available: " + updateAvailable);
        wordSprites[10]->show(0, 0, 14);
        wordSprites[11]->show(12 * 8, 0, 14);
        showingUpdateNotification = true;
    }

    SimpleListScene::update();
}
...
void initFallingBlocks() {
    memset16(backgroundArray, 0, 30 * bgGridHeight);
    memset32(backgroundTileCache, -1, 32 * 30);
}

#ifndef MULTIBOOT
void checkCustomSkins() {
    for (auto& customSkin : savefile->customSkins) {
        bool changed = false;

        int* skin = (int*)&customSkin.board;
        ...
        skin = (int*)&customSkin.smallBoard;
        ...
        customSkin.changed = changed && sum;
    }
}
#endif
